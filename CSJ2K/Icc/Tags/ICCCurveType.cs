...
public class ICCCurveType : ICCTag
{
    ...
    /// </param>
    protected internal ICCCurveType(int signature, byte[] data, int offset, int length) : base(signature, data, offset,
        offset + 2 * ICCProfile.int_size)
    {
    ...
    {
        var rep = new StringBuilder("[").Append(base.ToString()).Append(" nentries = ")
            .Append(Convert.ToString(nEntries))
            .Append(", length = " + Convert.ToString(entry_Renamed_Field.Length) + " ... ");
