...
public class ICCCurveTypeReverse : ICCTag
{
    ...
    /// </param>
    protected internal ICCCurveTypeReverse(int signature, byte[] data, int offset, int length) : base(signature, data,
        offset, offset + 2 * ICCProfile.int_size)
    {
        ...
        for (var i = 0; i < nEntries; ++i)
            ...
            entry_Renamed_Field[nEntries - 1 + i] =
                ICCProfile.getShort(data, offset + 3 * ICCProfile.int_size + i * ICCProfile.short_size) & 0xFFFF;
    }
    ...
    {
        var rep = new StringBuilder("[").Append(base.ToString()).Append(eol);
        rep.Append("num entries = " + Convert.ToString(nEntries) + eol);
        rep.Append("data length = " + Convert.ToString(entry_Renamed_Field.Length) + eol);
        for (var i = 0; i < nEntries; ++i)
