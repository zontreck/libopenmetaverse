...
public abstract class LookUpTable16 : LookUpTable
{
    ...
    /// <param name="dwNumInput">
    ///     the size of the lut t lut.
    /// </param>
    /// <param name="dwMaxOutput">
    ...
    {
        var rep = new StringBuilder("[LookUpTable16 ");
        //int row, col;
    ...
    {
        var rep = new StringBuilder("[LookUpTable16" + eol);
        int row, col;
        ...
        for (row = 0; row < dwNumInput / 10; ++row)
        {
            ...
            for (col = 0; col < 10; ++col) rep.Append(lut[10 * row + col]).Append(" ");
            rep.Append(eol);
        }
