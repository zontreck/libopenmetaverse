...
public class MatrixBasedTransformTosRGB
{
    ...
    ///     input profile
    /// </param>
    /// <param name="dwMaxValue">
    ///     clipping value for output.
    /// </param>
    /// <param name="dwMaxCols">
    ///     number of columns to transform
    /// </param>
    /// <param name="dwMaxRows">
    ...
    public MatrixBasedTransformTosRGB(RestrictedICCProfile ricc, int[] dwMaxValue, int[] dwShiftValue)
    {
        ...
        for (c = 0; c < 3; ++c) fLut[c] = LookUpTableFP.createInstance(ricc.trc[c], dwMaxValue[c] + 1);
        ...
        lut = LookUpTable32LinearSRGBtoSRGB.createInstance(dwMaxValue[0], dwMaxValue[0], ksRGBShadowCutoff,
            ksRGBShadowSlope, ksRGBScaleAfterExp, ksRGBExponent, ksRGBReduceAfterExp);
    }
    ...
    public override string ToString()
    {
        ...
        var rep = new StringBuilder("[MatrixBasedTransformTosRGB: ");

        var body = new StringBuilder("  ");
        body.Append(eol).Append("ksRGBExponent= ").Append(Convert.ToString(ksRGBExponent));
        body.Append(eol).Append("ksRGBScaleAfterExp= ").Append(Convert.ToString(ksRGBScaleAfterExp));
        body.Append(eol).Append("ksRGBReduceAfterExp= ").Append(Convert.ToString(ksRGBReduceAfterExp));


        body.Append(eol).Append("dwMaxValues= ").Append(Convert.ToString(dwMaxValue[0])).Append(", ")
            .Append(Convert.ToString(dwMaxValue[1])).Append(", ").Append(Convert.ToString(dwMaxValue[2]));

        body.Append(eol).Append("dwShiftValues= ").Append(Convert.ToString(dwShiftValue[0])).Append(", ")
            .Append(Convert.ToString(dwShiftValue[1])).Append(", ").Append(Convert.ToString(dwShiftValue[2]));

        //UPGRADE_TODO: The equivalent in .NET for method 'java.lang.Object.toString' may return a different value. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1043'"
        body.Append(eol).Append(eol).Append("fLut= ").Append(eol)
            .Append(ColorSpace.indent("  ", "fLut[RED]=  " + fLut[0])).Append(eol)
            .Append(ColorSpace.indent("  ", "fLut[GRN]=  " + fLut[1])).Append(eol)
            .Append(ColorSpace.indent("  ", "fLut[BLU]=  " + fLut[2]));
        ...
        for (i = 0; i < 3; ++i)
        {
            ...
            for (j = 0; j < 3; ++j) body.Append(matrix[3 * i + j] + "   ");
        }
        ...
        body.Append(eol).Append(eol).Append(lut);
    ...
    public virtual void apply(DataBlkInt[] inb, DataBlkInt[] outb)
    {
        ...
        if (fBuf == null || fBuf[0].Length < ncols * nrows)
        {
            ...
            for (var i = 0; i < 3; i++) tmpArray[i] = new float[ncols * nrows];
            fBuf = tmpArray;
        }
    ...
    public virtual void apply(DataBlkFloat[] inb, DataBlkFloat[] outb)
    {
        ...
        for (var y = 0; y < inb[0].h; ++y)
        {
            ...
            while (index < end)
            {
                ...
                // final LUT.
                //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                val = (int)(matrix[M00] * fBuf[RED][index] + matrix[M01] * fBuf[GREEN][index] +
                            matrix[M02] * fBuf[BLUE][index] + 0.5);
                // Clip the calculated value if necessary..
                if (val < 0)
                ...
                //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                val = (int)(matrix[M10] * fBuf[RED][index] + matrix[M11] * fBuf[GREEN][index] +
                            matrix[M12] * fBuf[BLUE][index] + 0.5);
                // Clip the calculated value if necessary..
                if (val < 0)
                ...
                //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                val = (int)(matrix[M20] * fBuf[RED][index] + matrix[M21] * fBuf[GREEN][index] +
                            matrix[M22] * fBuf[BLUE][index] + 0.5);
                // Clip the calculated value if necessary..
                if (val < 0)
    ...
    private static void standardizeMatrixLineThroughLut(DataBlkInt inb, float[] out_Renamed, int dwInputMaxValue,
        LookUpTableFP lut)
    {
        int wTemp, j = 0;
    ...
    private static void standardizeMatrixLineThroughLut(DataBlkFloat inb, float[] out_Renamed, float dwInputMaxValue,
        LookUpTableFP lut)
    {
        var j = 0;
        float wTemp;
