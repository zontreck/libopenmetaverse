...
public class PalettizedColorSpaceMapper : ColorSpaceMapper
{
    ...
    private void initialize()
    {
        ...
        for (var i = 0; i < outComps; i++) outShiftValueArray[i] = 1 << (getNomRangeBits(i) - 1);
    }
    ...
    public override DataBlk getCompData(DataBlk out_Renamed, int c)
    {
        ...
        if (ncomps != 1)
        {
            var msg = "PalettizedColorSpaceMapper: color palette " + "_not_ applied, incorrect number (" +
                      Convert.ToString(ncomps) + ") of components";
            ...
            case DataBlk.TYPE_INT:
                ...
                for (var row = 0; row < out_Renamed.h; ++row)
                {
                    ...
                    for (kOut = leftedgeOut, kIn = leftedgeIn; kIn < rightedgeIn; ++kIn, ++kOut)
                        outdataInt[kOut] = pbox.getEntry(c, dataInt[0][kIn] + shiftValueArray[0]) -
            ...
            case DataBlk.TYPE_FLOAT:
                ...
                for (var row = 0; row < out_Renamed.h; ++row)
                {
                    ...
                    for (kOut = leftedgeOut, kIn = leftedgeIn; kIn < rightedgeIn; ++kIn, ++kOut)
                        ...
                        outdataFloat[kOut] = pbox.getEntry(c, (int)dataFloat[0][kIn] + shiftValueArray[0]) -
                                             outShiftValueArray[c];
                }
    ...
    {
        int c;
        var rep = new StringBuilder("[PalettizedColorSpaceMapper ");
        var body = new StringBuilder("  " + eol);

        if (pbox != null)
        {
            body.Append("ncomps= ").Append(NumComps).Append(", scomp= ").Append(srcChannel);
            for (c = 0; c < NumComps; ++c)
                body.Append(eol).Append("column= ").Append(c).Append(", ").Append(pbox.getBitDepth(c)).Append(" bit ")
    ...
    ///     component. If this number is
    ///     <i>
    ///         n</b> then for unsigned data the
    ...
    /// </param>
    /// <returns>
    ///     The height in pixels of component <tt>c</tt> in tile
    ///     <tt>t</tt>.
    /// </returns>
