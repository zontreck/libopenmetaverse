...
{
    private readonly Hashtable definitions = Hashtable.Synchronized(new Hashtable());
    ...
    {
        var bfr = (int[])definitions[channel];
        return getTyp(bfr);
    }
    ...
    {
        var rep = new StringBuilder("[ChannelDefinitionBox ").Append(eol).Append("  ");
        ...
        while (keys.MoveNext())
        {
            //UPGRADE_TODO: Method 'java.util.Enumeration.nextElement' was converted to 'System.Collections.IEnumerator.Current' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javautilEnumerationnextElement'"
            var bfr = (int[])definitions[keys.Current];
            rep.Append(eol).Append("  ").Append("Cn= ").Append(Convert.ToString(getCn(bfr))).Append(", ")
                .Append("Typ= ").Append(Convert.ToString(getTyp(bfr))).Append(", ").Append("Asoc= ")
