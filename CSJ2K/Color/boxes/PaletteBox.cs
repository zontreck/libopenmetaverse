...
public sealed class PaletteBox : JP2Box
{
    ...
    internal void readBox()
    {
        ...
        for (i = 0; i < NumColumns; ++i) bitdepth[i] = (short)(bfr[i] & 0x00fff);
    ...
    {
        var rep = new StringBuilder("[PaletteBox ").Append("nentries= ").Append(Convert.ToString(NumEntries))
            .Append(", ncolumns= ").Append(Convert.ToString(NumColumns)).Append(", bitdepth per column= (");
