...
public abstract class ColorSpaceMapper : ImgDataAdapter, BlkImgDataSrc
{
    ...
    private static readonly string[][] pinfo =
        { new[] { "IcolorSpacedebug", null, "Print debugging messages during colorspace mapping.", "off" } };
    ...
    public static BlkImgDataSrc createInstance(BlkImgDataSrc src, ColorSpace csMap)
    {
        ...
        if (csMap.Method == ColorSpace.MethodEnum.ICC_PROFILED) return ICCProfiler.createInstance(src, csMap);
        ...
        if (colorspace == ColorSpace.CSEnum.GreyScale)
            return EnumeratedColorSpaceMapper.createInstance(src, csMap);
        if (colorspace == ColorSpace.CSEnum.sYCC) return SYccColorSpaceMapper.createInstance(src, csMap);
