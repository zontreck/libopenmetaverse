...
public class ISRandomAccessIO : RandomAccessIO
{
    ...
    * has been reached. */
    ...
    {
        if (size < 0 || inc <= 0 || maxsize <= 0 || is_Renamed == null) throw new ArgumentException();
    ...
    ///     The input from where to get the data.
    /// </param>
    public ISRandomAccessIO(Stream is_Renamed) : this(is_Renamed, 1 << 18, 1 << 18, int.MaxValue)
    {
    }
    ...
    public virtual byte read()
    {
        if (pos < len)
            ...
            return buf[pos++];
        // general case
        while (!complete && pos >= len) readInput();
        if (pos == len)
            throw new EndOfStreamException();
        if (pos > len) throw new IOException("Position beyond EOF");
        return buf[pos++];
    }
    ...
    public virtual void readFully(byte[] b, int off, int n)
    {
        ...
        // general case
        while (!complete && pos + n > len) readInput();
        if (pos + n > len) throw new EndOfStreamException();
        Array.Copy(buf, pos, b, off, n);
        pos += n;
    }
    ...
    public virtual long readUnsignedInt()
    {
        if (pos + 3 < len)
            // common, fast case
            return unchecked((int)0xFFFFFFFFL) & (long)((buf[pos++] << 24) | ((0xFF & buf[pos++]) << 16) |
    ...
    public virtual long readLong()
    {
        if (pos + 7 < len)
            // common, fast case
            return ((long)buf[pos++] << 56) | ((long)buf[pos++] << 48) | ((long)buf[pos++] << 40) |
                   ((long)buf[pos++] << 32) | ((long)buf[pos++] << 24) | ((long)buf[pos++] << 16) |
                   ((long)buf[pos++] << 8) | buf[pos++];
        // general case
        return ((long)read() << 56) | ((long)read() << 48) | ((long)read() << 40) | ((long)read() << 32) |
               ((long)read() << 24) | ((long)read() << 16) | ((long)read() << 8) | read();
    }
    ...
    public virtual float readFloat()
    {
        ...
        int floatint;
        if (pos + 3 < len)
            floatint = (buf[pos++] << 24) | ((0xFF & buf[pos++]) << 16) | ((0xFF & buf[pos++]) << 8) |
                       (0xFF & buf[pos++]);
        else
    ...
    public virtual double readDouble()
    {
        ...
        long doublelong;
        if (pos + 7 < len)
            doublelong = ((long)buf[pos++] << 56) | ((long)buf[pos++] << 48) | ((long)buf[pos++] << 40) |
                         ((long)buf[pos++] << 32) | ((long)buf[pos++] << 24) | ((long)buf[pos++] << 16) |
                         ((long)buf[pos++] << 8) | buf[pos++];
        else
            doublelong = ((long)read() << 56) | ((long)read() << 48) | ((long)read() << 40) | ((long)read() << 32) |
                         ((long)read() << 24) | ((long)read() << 16) | ((long)read() << 8) | read();
    ...
    private void growBuffer()
    {
        ...
        if (effinc <= 0) throw new IOException("Reached maximum cache size (" + maxsize + ")");
        try
        {
    ...
    private void readInput()
    {
        ...
        if (complete) throw new ArgumentException("Already reached EOF");
        long available;
