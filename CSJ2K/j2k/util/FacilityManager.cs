...
public class FacilityManager
{
    ...
    private static readonly Hashtable loggerList = Hashtable.Synchronized(new Hashtable());

    /// <summary>The default logger, for threads that have none associated with them </summary>
    private static MsgLogger defMsgLogger =
        new StreamMsgLogger(Console.OpenStandardOutput(), Console.OpenStandardError(), 78);
    ...
    private static readonly Hashtable watchProgList = Hashtable.Synchronized(new Hashtable());
    ...
    public static ProgressWatch ProgressWatch
    {
        get
        {
            var pw = (ProgressWatch)watchProgList[SupportClass.ThreadClass.Current()];
    ...
    {
        if (pw == null) throw new NullReferenceException();
        if (t == null)
            defWatchProg = pw;
        else
            watchProgList[t] = pw;
    ...
    {
        if (ml == null) throw new NullReferenceException();
        if (t == null)
            defMsgLogger = ml;
        else
            loggerList[t] = ml;
    ...
    {
        var ml = (MsgLogger)loggerList[SupportClass.ThreadClass.Current()];
