...
public class CodestreamManipulator
{
    ...
    private void parseAndFind(BufferedRandomAccessFile fi)
    {
        ...
        int halfMarker;
        int tileEnd;
        var markPos = ArrayList.Synchronized(new ArrayList(10));
    ...
    private void createTileParts()
    {
        ...
        for (t = 0; t < nt; t++)
        {
            ...
            numTileParts = (int)Math.Ceiling((double)prem / pptp);
            ...
            for (tilePart = 0; tilePart < numTileParts; tilePart++)
            {
                ...
                if (pptUsed)
                {
                    ...
                    while (np > 0)
                    {
                        ...
                        {
                            temp.WriteByte((byte)SupportClass.URShift(Markers.PPT, 8));
                            temp.WriteByte(Markers.PPT & 0x00FF);
                            temp.WriteByte((byte)SupportClass.URShift(pptLength, 8));
                            temp.WriteByte((byte)pptLength);
                            temp.WriteByte((byte)pptIndex++);
                            for (i = pIndex; i < p; i++) temp.Write(packetHeaders[t][i], 0, packetHeaders[t][i].Length);
                ...
                // Write SOD marker
                temp.WriteByte((byte)SupportClass.URShift(Markers.SOD, 8));
                temp.WriteByte(Markers.SOD & 0x00FF);

                // Write packet data and packet headers if PPT and PPM not used
                for (p = tppStart; p < tppStart + np; p++)
                {
                    if (!tempSop) temp.Write(sopMarkSeg[t][p], 0, Markers.SOP_LENGTH);

                    if (!(ppmUsed || pptUsed)) temp.Write(packetHeaders[t][p], 0, packetHeaders[t][p].Length);
                ...
                {
                    // Edit tile part header
                    tempByteArr[0] = (byte)SupportClass.URShift(Markers.SOT, 8); // SOT
                    tempByteArr[1] = Markers.SOT & 0x00FF;
    ...
    private void writeNewCodestream(BufferedRandomAccessFile fi)
    {
        ...
        for (var i2 = 0; i2 < numTiles; i2++) packetHeaderLengths[i2] = new int[maxtp];
        byte[] temp;
        int length;
        ...
        if (ppmUsed)
        {
            ...
            for (t = 0; t < numTiles; t++) prem[t] = packetHeaders[t].Length;
            ...
            ppmMarkerSegment.WriteByte((byte)SupportClass.URShift(Markers.PPM, 8));
            ppmMarkerSegment.WriteByte(Markers.PPM & 0x00FF);
