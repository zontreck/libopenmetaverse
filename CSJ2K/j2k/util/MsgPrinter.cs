...
public class MsgPrinter
{
    ...
    public virtual void print(StreamWriter out_Renamed, int flind, int ind, string msg)
    {
        ...
        while ((end = nextLineEnd(msg, pend)) != IS_EOS)
        {
            ...
            {
                // Forced line break
                for (i = 0; i < lind; i++) out_Renamed.Write(" ");
            ...
            pend = start;
            if (start == IS_EOS) break; // Did all the string
        }
    ...
    private int nextLineEnd(string str, int from)
    {
        ...
        while (from < len && (c = str[from]) != '\n' && char.IsWhiteSpace(c)) from++;
        if (c == '\n') return IS_NEWLINE;
        if (from >= len) return IS_EOS;
        // Now skip word characters
        while (from < len && !char.IsWhiteSpace(str[from])) from++;
        return from;
    }
