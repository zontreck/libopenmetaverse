...
public class ThreadPool
{
    ...
    private volatile ApplicationException targetE;

    /// <summary>The last runtime exception thrown by a target. Null if none </summary>
    // NOTE: needs to be volatile, so that only one copy exits in memory
    private volatile SystemException targetRE;
    ...
    public ThreadPool(int size, int priority, string name)
    {
        ...
        if (size <= 0) throw new ArgumentException("Pool must be of positive size");
        if (priority < (int)ThreadPriority.Lowest)
            poolPriority = (int)SupportClass.ThreadClass.Current().Priority;
        else
            poolPriority = priority < (int)ThreadPriority.Highest ? priority : (int)ThreadPriority.Highest;
        if (name == null)
    ...
    ///     The pool's size.
    /// </returns>
    public virtual int Size => idle.Length;
    ...
    internal class ThreadPoolThread : SupportClass.ThreadClass
    {
        ...
        /// </param>
        public ThreadPoolThread(ThreadPool enclosingInstance, int idx, string name) : base(name)
        {
            ...
            Priority = (ThreadPriority)Enclosing_Instance.poolPriority;
        }

        public ThreadPool Enclosing_Instance { get; private set; }
            ...
            lock (this)
            {
                while (true)
                {
                    ...
                    catch (Exception)
                    {
                        ...
                        // never be.
                        Enclosing_Instance.targetRE = new SystemException("Unchecked exception " +
                                                                          "thrown by target's " +
                                                                          "run() method in pool " +
                                                                          Enclosing_Instance.poolName + ".");
                    }
        ...
        internal virtual void setTarget(IThreadRunnable target, object lock_Renamed, bool notifyAll)
        {
            lock (this)
            {
                // Set the target
