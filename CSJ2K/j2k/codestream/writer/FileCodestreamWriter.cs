...
public class FileCodestreamWriter : CodestreamWriter
{
    ...
    /// </exception>
    public FileCodestreamWriter(FileInfo file, int mb) : base(mb)
    {
        ...
        out_Renamed = new BufferedStream(new FileStream(file.FullName, FileMode.Create), DEF_BUF_LEN);
        initSOP_EPHArrays();
    }
    ...
    ///     for writing or while writing the magic number.
    /// </exception>
    public FileCodestreamWriter(string fname, int mb) : base(mb)
    {
        //UPGRADE_TODO: Constructor 'java.io.FileOutputStream.FileOutputStream' was converted to 'System.IO.FileStream.FileStream' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javaioFileOutputStreamFileOutputStream_javalangString'"
        out_Renamed = new BufferedStream(new FileStream(fname, FileMode.Create), DEF_BUF_LEN);
        initSOP_EPHArrays();
    }
    ...
    /// </exception>
    public FileCodestreamWriter(Stream os, int mb) : base(mb)
    {
        out_Renamed = os;
        initSOP_EPHArrays();
    }
    ...
    public override int writePacketHead(byte[] head, int hlen, bool sim, bool sop, bool eph)
    {
        ...
        if (!sim)
        {
            ...
            if (MaxAvailableBytes < len) len = MaxAvailableBytes;
            ...
            if (len > 0)
            {
                ...
                if (eph) out_Renamed.Write(ephMarker, 0, Markers.EPH_LENGTH);
    ...
    public override int writePacketBody(byte[] body, int blen, bool sim, bool roiInPkt, int roiLen)
    {
        ...
        if (!sim)
        {
            ...
            len = blen;
            if (MaxAvailableBytes < len) len = MaxAvailableBytes;
            if (blen > 0) out_Renamed.Write(body, 0, len);
            // Update data length 
            ndata += len;
