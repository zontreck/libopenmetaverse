...
public class PktEncoder
{
    ...
    private static readonly string[][] pinfo =
    {
        new[]
        {
            "Psop", "[<tile idx>] on|off" + "[ [<tile idx>] on|off ...]",
            "Specifies whether start of packet (SOP) markers should be used. " + "'on' enables, 'off' disables it.",
            "off"
        },
        new[]
        {
            "Peph", "[<tile idx>] on|off" + "[ [<tile  idx>] on|off ...]",
    ...
    public PktEncoder(CodedCBlkDataSrcEnc infoSrc, EncoderSpecs encSpec, Coord[][][] numPrec, ParameterList pl)
    {
        ...
        for (var i = 0; i < nt; i++) ttIncl[i] = new TagTreeEncoder[nc][][][];
        ttMaxBP = new TagTreeEncoder[nt][][][][];
        for (var i2 = 0; i2 < nt; i2++) ttMaxBP[i2] = new TagTreeEncoder[nc][][][];
        lblock = new int[nt][][][][];
        for (var i3 = 0; i3 < nt; i3++) lblock[i3] = new int[nc][][][];
        prevtIdxs = new int[nt][][][][];
        for (var i4 = 0; i4 < nt; i4++) prevtIdxs[i4] = new int[nc][][][];
        ppinfo = new PrecInfo[nt][][][];
        for (var i5 = 0; i5 < nt; i5++) ppinfo[i5] = new PrecInfo[nc][][];
        ...
        for (var t = 0; t < nt; t++)
        {
            ...
            for (var c = 0; c < nc; c++)
            {
                ...
                for (var r = 0; r <= mrl; r++)
                {
                    ...
                    for (var i6 = 0; i6 < maxPrec; i6++) ttIncl[t][c][r][i6] = new TagTreeEncoder[maxs];
                    ttMaxBP[t][c][r] = new TagTreeEncoder[maxPrec][];
                    for (var i7 = 0; i7 < maxPrec; i7++) ttMaxBP[t][c][r][i7] = new TagTreeEncoder[maxs];
    ...
    public virtual byte[] LastBodyBuf
    {
        get
        {
            if (lbbuf == null) throw new ArgumentException();
            return lbbuf;
        }
    }
    ...
    private void fillPrecInfo(int t, int c, int r)
    {
        ...
        for (var j = jstart; j <= jend; j++, nPrec++)
        {
            ...
            //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
            ppinfo[t][c][r][nPrec] = new PrecInfo(r, (int)(cb0x + j * twoppx), (int)(cb0y + i * twoppy), (int)twoppx,
    ...
    /// </returns>
    public virtual BitOutputBuffer encodePacket(int ly, int c, int r, int t, CBlkRateDistStats[][] cbs, int[][] tIndx,
        BitOutputBuffer hbuf, byte[] bbuf, int pIdx)
    {
        ...
        if (isPrecVoid)
        {
            ...
            if (hbuf == null)
                ...
                hbuf.reset();
            if (bbuf == null) lbbuf = bbuf = new byte[1];
            hbuf.writeBit(0);
            lblen = 0;
            ...
            for (var n = 0; n < prec.cblk[s][m].Length; n++)
            {
                ...
                if (cur_tIndx[b] > cur_prevtIdxs[b])
                {
                    ...
                    if (cur_prevtIdxs[b] < 0)
                    {
                        ...
                        for (i = 1; i <= thmax; i++) cur_ttMaxBP.encode(m, n, i, hbuf);
        ...
        if (bbuf == null || bbuf.Length < lblen) bbuf = new byte[lblen];
        lbbuf = bbuf;
        lblen = 0;
                ...
                for (var n = 0; n < nend; n++)
                {
                    ...
                    if (cur_tIndx[b] > cur_prevtIdxs[b])
                    {
                        ...
                        else
                        {
                            cblen = cur_cbs[b].truncRates[cur_cbs[b].truncIdxs[cur_tIndx[b]]] -
                                    cur_cbs[b].truncRates[cur_cbs[b].truncIdxs[cur_prevtIdxs[b]]];
                            Array.Copy(cur_cbs[b].data, cur_cbs[b].truncRates[cur_cbs[b].truncIdxs[cur_prevtIdxs[b]]],
                                lbbuf, lblen, cblen);
                        }
                        ...
                        // information
                        if (cur_cbs[b].nROIcoeff != 0 && (cur_prevtIdxs[b] == -1 ||
        ...
        // Must never happen
        if (hbuf.Length == 0)
            throw new ApplicationException("You have found a bug in PktEncoder, method:" + " encodePacket");
    ...
    public virtual void restore()
    {
        ...
        if (!saved) throw new ArgumentException();
