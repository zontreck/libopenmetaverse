...
public class TagTreeEncoder
{
    ...
    public TagTreeEncoder(int h, int w)
    {
        ...
        init(w, h);
        // Set values to max
        for (k = treeV.Length - 1; k >= 0; k--) ArrayUtil.intArraySet(treeV[k], int.MaxValue);
    }
    ...
    public TagTreeEncoder(int h, int w, int[] val)
    {
        int k;
        // Check arguments
        if (w < 0 || h < 0 || val.Length < w * h) throw new ArgumentException();
        // Initialize elements
        init(w, h);
        // Update leaf values
        for (k = w * h - 1; k >= 0; k--) treeV[0][k] = val[k];
    ...
    public virtual int[] Values
    {
        set
        {
            ...
            for (i = w * h - 1; i >= 0; i--)
            {
                if ((treeV[0][i] < maxt || value[i] < maxt) && treeV[0][i] != value[i]) throw new ArgumentException();
                // Update leaf value
                treeV[0][i] = value[i];
            }
            ...
            for (m = ((lh >> 1) << 1) - 2; m >= 0; m -= 2)
            {
                ...
                if (lw % 2 != 0)
                {
                    ...
                    treeV[k + 1][(m >> 1) * ((lw + 1) >> 1) + (n >> 1)] =
                        treeV[k][bi] < treeV[k][bi + lw] ? treeV[k][bi] : treeV[k][bi + lw];
                }
            }
    ...
    public virtual void encode(int m, int n, int t, BitOutputBuffer out_Renamed)
    {
        ...
        // Check arguments
        if (m >= h || n >= w || t < 0) throw new ArgumentException();
    ...
    public virtual void reset(int[] val)
    {
        ...
        recalcTreeV();
        // Set all states to 0
        for (k = lvls - 1; k >= 0; k--) ArrayUtil.intArraySet(treeS[k], 0);
        // Invalidate saved tree
        saved = false;
    }
}