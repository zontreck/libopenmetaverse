...
public class HeaderEncoder
{
    ...
    {
        new[] { "Hjj2000_COM", null, "Writes or not the JJ2000 COM marker in the " + "codestream", "off" },
        new[]
        {
    ...
    protected internal BinaryWriter hbuf;
    ...
    /// </param>
    public HeaderEncoder(ImgData origsrc, bool[] isorigsig, ForwardWT dwt, Tiler tiler, EncoderSpecs encSpec,
        ROIScaler roiSc, PostCompRateAllocator ralloc, ParameterList pl)
    {
        pl.checkList(OPT_PREFIX, ParameterList.toNameArray(pinfo));
        if (origsrc.NumComps != isorigsig.Length) throw new ArgumentException();
        origSrc = origsrc;
        isOrigSig = isorigsig;
        this.dwt = dwt;
        this.tiler = tiler;
        ...
        //UPGRADE_TODO: Class 'java.io.DataOutputStream' was converted to 'System.IO.BinaryWriter' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javaioDataOutputStream'"
        hbuf = new EndianBinaryWriter(baos, true);
    ...
    public virtual void reset()
    {
        ...
        baos.SetLength(0);
        //UPGRADE_TODO: Class 'java.io.DataOutputStream' was converted to 'System.IO.BinaryWriter' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javaioDataOutputStream'"
        hbuf = new EndianBinaryWriter(baos, true); //new System.IO.BinaryWriter(baos);
    }
    ...
    public virtual void writeTo(BinaryDataOutput out_Renamed)
    {
        ...
        for (i = 0; i < len; i++) out_Renamed.writeByte(buf[i]);
    }
    ...
    ///     Where to write the header.
    /// </param>
    public virtual void writeTo(Stream out_Renamed)
    {
    ...
    protected internal virtual void writeCOD(bool mh, int tileIdx)
    {
        ...
        tmp = 0;
        if (precinctPartitionUsed) tmp = Markers.SCOX_PRECINCT_PARTITION;

        // Are SOP markers used ?
        if (mh)
        {
            //UPGRADE_TODO: The equivalent in .NET for method 'java.lang.Object.toString' may return a different value. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1043'"
            if (encSpec.sops.getDefault().ToString().ToUpper().Equals("on".ToUpper())) tmp |= Markers.SCOX_USE_SOP;
        }
        else
        {
            //UPGRADE_TODO: The equivalent in .NET for method 'java.lang.Object.toString' may return a different value. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1043'"
        ...
        {
            //UPGRADE_TODO: The equivalent in .NET for method 'java.lang.Object.toString' may return a different value. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1043'"
            if (encSpec.ephs.getDefault().ToString().ToUpper().Equals("on".ToUpper())) tmp |= Markers.SCOX_USE_EPH;
        }
        else
        {
            //UPGRADE_TODO: The equivalent in .NET for method 'java.lang.Object.toString' may return a different value. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1043'"
        ...
        if (mh)
        {
            ...
            if (((string)encSpec.bms.getDefault()).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_BYPASS;
            // MQ reset after each coding pass ?
            if (((string)encSpec.mqrs.getDefault()).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_RESET_MQ;
            // MQ termination after each arithmetically coded coding pass ?
            if (((string)encSpec.rts.getDefault()).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_TERM_PASS;
            // Vertically stripe-causal context mode ?
            if (((string)encSpec.css.getDefault()).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_VERT_STR_CAUSAL;
            // Predictable termination ?
            if (((string)encSpec.tts.getDefault()).Equals("predict")) tmp |= StdEntropyCoderOptions.OPT_PRED_TERM;
            // Error resilience segmentation symbol insertion ?
            if (((string)encSpec.sss.getDefault()).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_SEG_SYMBOLS;
        }
        else
        {
            // Tile header
            // Selective arithmetic coding bypass ?
            if (((string)encSpec.bms.getTileDef(tileIdx)).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_BYPASS;
            // MQ reset after each coding pass ?
            if (((string)encSpec.mqrs.getTileDef(tileIdx)).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_RESET_MQ;
            // MQ termination after each arithmetically coded coding pass ?
            if (((string)encSpec.rts.getTileDef(tileIdx)).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_TERM_PASS;
            // Vertically stripe-causal context mode ?
            ...
            // Error resilience segmentation symbol insertion ?
            if (((string)encSpec.sss.getTileDef(tileIdx)).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_SEG_SYMBOLS;
        }
        ...
        if (precinctPartitionUsed)
        {
            ...
            ArrayList[] v = null;
            if (mh)
                v = (ArrayList[])encSpec.pss.getDefault();
            else
                v = (ArrayList[])encSpec.pss.getTileDef(tileIdx);
            for (var r = mrl; r >= 0; r--)
            {
                if (r >= v[1].Count)
                    tmp = (int)v[1][v[1].Count - 1];
                else
                    tmp = (int)v[1][r];
                var yExp = (MathUtil.log2(tmp) << 4) & 0x00F0;

                if (r >= v[0].Count)
                    tmp = (int)v[0][v[0].Count - 1];
                else
                    tmp = (int)v[0][r];
    ...
    protected internal virtual void writeCOC(bool mh, int tileIdx, int compIdx)
    {
        ...
        if (mh)
        {
            // Main header
            // Lazy coding mode ?
            if (((string)encSpec.bms.getCompDef(compIdx)).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_BYPASS;
            // MQ reset after each coding pass ?
            if (((string)encSpec.mqrs.getCompDef(compIdx)).ToUpper().Equals("on".ToUpper()))
                tmp |= StdEntropyCoderOptions.OPT_RESET_MQ;
            // MQ termination after each arithmetically coded coding pass ?
            if (((string)encSpec.rts.getCompDef(compIdx)).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_TERM_PASS;
            // Vertically stripe-causal context mode ?
            ...
            if (((string)encSpec.tts.getCompDef(compIdx)).Equals("predict"))
                tmp |= StdEntropyCoderOptions.OPT_PRED_TERM;
            // Error resilience segmentation symbol insertion ?
            if (((string)encSpec.sss.getCompDef(compIdx)).Equals("on")) tmp |= StdEntropyCoderOptions.OPT_SEG_SYMBOLS;
        }
        else
        {
            // Tile Header
        ...
        if (precinctPartitionUsed)
        {
            ...
            ArrayList[] v = null;
            if (mh)
                v = (ArrayList[])encSpec.pss.getCompDef(compIdx);
            else
                v = (ArrayList[])encSpec.pss.getTileCompVal(tileIdx, compIdx);
            for (var r = mrl; r >= 0; r--)
            {
                if (r >= v[1].Count)
                    tmp = (int)v[1][v[1].Count - 1];
    ...
    protected internal virtual void writeMainQCD()
    {
        ...
        if (notFound)
            throw new ApplicationException("Default representative for quantization type " +
        ...
        qstyle = isReversible ? Markers.SQCX_NO_QUANTIZATION :
            isDerived ? Markers.SQCX_SCALAR_DERIVED : Markers.SQCX_SCALAR_EXPOUNDED;
        ...
        switch (qstyle)
        {
            ...
            default:
                throw new ApplicationException("Internal JJ2000 error");
        }
    ...
    protected internal virtual void writeMainQCC(int compIdx)
    {
        ...
        if (notFound)
            throw new ApplicationException("Default representative for quantization type " +
                                           " and number of decomposition levels not found " + " in main QCC (c=" +
                                           compIdx + ") marker segment. " + "You have found a JJ2000 bug.");
        ...
        switch (qstyle)
        {
            ...
            case Markers.SQCX_SCALAR_EXPOUNDED:
                ...
                while (sb.resLvl != 0) sb = sb.subb_LL;
    ...
    protected internal virtual void writeTileQCD(int tIdx)
    {
        ...
        if (notFound)
            throw new ApplicationException("Default representative for quantization type " +
                                           " and number of decomposition levels not found " + " in tile QCD (t=" +
                                           tIdx + ") marker segment. " + "You have found a JJ2000 bug.");
    ...
    public virtual void encodeMainHeader()
    {
        ...
        for (i = 0; i < nComp; i++)
        {
            ...
            if (encSpec.wfs.isCompSpecified(i) || encSpec.dls.isCompSpecified(i) || encSpec.bms.isCompSpecified(i) ||
                encSpec.mqrs.isCompSpecified(i) || encSpec.rts.isCompSpecified(i) || encSpec.sss.isCompSpecified(i) ||
                encSpec.css.isCompSpecified(i) || encSpec.pss.isCompSpecified(i) || encSpec.cblks.isCompSpecified(i) ||
        ...
        for (i = 0; i < nComp; i++)
            if (dwt.getNomRangeBits(i) != defimgn || encSpec.qts.isCompSpecified(i) ||
                encSpec.qsss.isCompSpecified(i) || encSpec.dls.isCompSpecified(i) || encSpec.gbs.isCompSpecified(i))
                writeMainQCC(i);
    ...
    private void writeCOM()
    {
        ...
        if (enJJ2KMarkSeg)
        {
            ...
            for (var i = 0; i < chars.Length; i++) hbuf.Write(chars[i]);
        }
    ...
    public virtual void encodeTilePartHeader(int tileLength, int tileIdx)
    {
        ...
        if (encSpec.wfs.isTileSpecified(tileIdx) || encSpec.cts.isTileSpecified(tileIdx) ||
            encSpec.dls.isTileSpecified(tileIdx) || encSpec.bms.isTileSpecified(tileIdx) ||
            encSpec.mqrs.isTileSpecified(tileIdx) || encSpec.rts.isTileSpecified(tileIdx) ||
            encSpec.css.isTileSpecified(tileIdx) || encSpec.pss.isTileSpecified(tileIdx) ||
            encSpec.sops.isTileSpecified(tileIdx) || encSpec.sss.isTileSpecified(tileIdx) ||
            encSpec.pocs.isTileSpecified(tileIdx) || encSpec.ephs.isTileSpecified(tileIdx) ||
            encSpec.cblks.isTileSpecified(tileIdx) || isEresUsed != isEresUsedInTile)
        {
        ...
        for (var c = 0; c < nComp; c++)
        {
            ...
            if (encSpec.wfs.isTileCompSpecified(tileIdx, c) || encSpec.dls.isTileCompSpecified(tileIdx, c) ||
                encSpec.bms.isTileCompSpecified(tileIdx, c) || encSpec.mqrs.isTileCompSpecified(tileIdx, c) ||
                encSpec.rts.isTileCompSpecified(tileIdx, c) || encSpec.css.isTileCompSpecified(tileIdx, c) ||
                encSpec.pss.isTileCompSpecified(tileIdx, c) || encSpec.sss.isTileCompSpecified(tileIdx, c) ||
                encSpec.cblks.isTileCompSpecified(tileIdx, c) || isEresUsedInTileComp != isEresUsed)
                writeCOC(false, tileIdx, c);
            else if (tileCODwritten)
                if (encSpec.wfs.isCompSpecified(c) || encSpec.dls.isCompSpecified(c) ||
                    encSpec.bms.isCompSpecified(c) || encSpec.mqrs.isCompSpecified(c) ||
                    encSpec.rts.isCompSpecified(c) || encSpec.sss.isCompSpecified(c) ||
                    encSpec.css.isCompSpecified(c) || encSpec.pss.isCompSpecified(c) ||
                    encSpec.cblks.isCompSpecified(c) || (encSpec.tts.isCompSpecified(c) &&
        ...
        if (encSpec.qts.isTileSpecified(tileIdx) || encSpec.qsss.isTileSpecified(tileIdx) ||
            encSpec.dls.isTileSpecified(tileIdx) || encSpec.gbs.isTileSpecified(tileIdx))
        {
        ...
        // +--------------------------+
        for (var c = 0; c < nComp; c++)
            if (dwt.getNomRangeBits(c) != deftilenr || encSpec.qts.isTileCompSpecified(tileIdx, c) ||
                encSpec.qsss.isTileCompSpecified(tileIdx, c) || encSpec.dls.isTileCompSpecified(tileIdx, c) ||
                encSpec.gbs.isTileCompSpecified(tileIdx, c))
                writeTileQCC(tileIdx, c);
            else if (tileQCDwritten)
                if (encSpec.qts.isCompSpecified(c) || encSpec.qsss.isCompSpecified(c) ||
                    encSpec.dls.isCompSpecified(c) || encSpec.gbs.isCompSpecified(c))
                    writeTileQCC(tileIdx, c);
