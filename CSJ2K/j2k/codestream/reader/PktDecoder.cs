...
public class PktDecoder
{
    ...
    /// </param>
    public virtual CBlkInfo[][][][][] restart(int nc, int[] mdl, int nl, CBlkInfo[][][][][] cbI, bool pph,
        MemoryStream pphbais)
    {
        ...
        for (var c = 0; c < nc; c++)
        {
            ...
            for (var r = 0; r <= mdl[c]; r++)
            {
                ...
                if (trx1 > trx0)
                    //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                    numPrec[c][r].x = (int)Math.Ceiling((trx1 - cb0x) / twoppx) -
                                      (int)Math.Floor((trx0 - cb0x) / twoppx);
                else
                    numPrec[c][r].x = 0;
                if (try1 > try0)
                    //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                    numPrec[c][r].y = (int)Math.Ceiling((try1 - cb0y) / twoppy) -
                ...
                for (var i = 0; i < maxPrec; i++) ttIncl[c][r][i] = new TagTreeDecoder[maxs + 1];
                ttMaxBP[c][r] = new TagTreeDecoder[maxPrec][];
                for (var i2 = 0; i2 < maxPrec; i2++) ttMaxBP[c][r][i2] = new TagTreeDecoder[maxs + 1];
                ...
                for (var s = mins; s < maxs; s++)
                {
                    ...
                    for (var i3 = 0; i3 < nBlk.y; i3++) cbI[c][r][s][i3] = new CBlkInfo[nBlk.x];
                    lblock[c][r][s] = new int[nBlk.y][];
                    for (var i4 = 0; i4 < nBlk.y; i4++) lblock[c][r][s][i4] = new int[nBlk.x];

                    for (var i = nBlk.y - 1; i >= 0; i--) ArrayUtil.intArraySet(lblock[c][r][s][i], INIT_LBLOCK);
                } // loop on subbands
    ...
    private void fillPrecInfo(int c, int r, int mdl)
    {
        ...
        for (var j = jstart; j <= jend; j++, nPrec++)
        {
            ...
            //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
            ppinfo[c][r][nPrec] = new PrecInfo(r, (int)(cb0x + j * twoppx), (int)(cb0y + i * twoppy), (int)twoppx,
                (int)twoppy, prg_ulx, prg_uly, prg_w, prg_h);
            ...
            if (r == 0)
            {
                ...
                else
                {
                    ...
                    for (var i2 = 0; i2 < kend - kstart + 1; i2++) tmpArray[i2] = new CBlkCoordInfo[lend - lstart + 1];
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (var i3 = 0; i3 < kend - kstart + 1; i3++) tmpArray2[i3] = new CBlkCoordInfo[lend - lstart + 1];
                ...
                else
                {
                    ...
                    for (var i4 = 0; i4 < kend - kstart + 1; i4++) tmpArray3[i4] = new CBlkCoordInfo[lend - lstart + 1];
                ...
                else
                {
                    ...
                    for (var i5 = 0; i5 < kend - kstart + 1; i5++) tmpArray4[i5] = new CBlkCoordInfo[lend - lstart + 1];
    ...
    public virtual bool readPktHead(int l, int r, int c, int p, CBlkInfo[][][] cbI, int[] nb)
    {
        ...
        if (bin.readBit() == 0)
        {
            ...
            for (var s = mins; s < maxs; s++) cblks[s] = ArrayList.Synchronized(new ArrayList(10));
            pktIdx++;
            ...
            if (ephUsed) readEPHMarker(bin);
            return false;
        }

        // Packet is not empty => decode info
        // Loop on each subband in this resolution level
        if (cblks == null || cblks.Length < maxs + 1) cblks = new ArrayList[maxs + 1];

        for (var s = mins; s < maxs; s++)
        {
            if (cblks[s] == null)
                cblks[s] = ArrayList.Synchronized(new ArrayList(10));
            else
                cblks[s].Clear();
            sb = (SubbandSyn)root.getSubbandByIdx(r, s);
            ...
            for (var m = 0; m < mend; m++)
            {
                ...
                for (var n = 0; n < nend; n++)
                {
                    ...
                    try
                    {
                        ...
                        if (ccb == null || ccb.ctp == 0)
                        {
                            if (ccb == null)
                                ccb = cbI[s][cbc.y][cbc.x] = new CBlkInfo(prec.cblk[s][m][n].ulx,
                                    prec.cblk[s][m][n].uly, prec.cblk[s][m][n].w, prec.cblk[s][m][n].h, nl);
                            ccb.pktIdx[l] = pktIdx;
                            ...
                            for (tmp2 = 1; tmp >= tmp2; tmp2++) tmp = tdBD.update(m, n, tmp2, bin);
                        ...
                        while (bin.readBit() != 0) lblock[c][r][s][cbc.y][cbc.x]++;
    ...
    public virtual bool readSOPMarker(int[] nBytes, int p, int c, int r)
    {
        ...
        val |= sopArray[1];
        if (val != Markers.SOP)
            throw new ApplicationException("Corrupted Bitstream: Could not parse SOP " + "marker !");

        // Check if length is correct
        val = sopArray[2] & 0xff;
        val <<= 8;
        val |= sopArray[3] & 0xff;
        if (val != 4) throw new ApplicationException("Corrupted Bitstream: Corrupted SOP marker !");
        ...
        if (!pph && val != pktIdx)
            throw new ApplicationException("Corrupted Bitstream: SOP marker out of " + "sequence !");
        if (pph && val != pktIdx - 1)
    ...
    public virtual void readEPHMarker(PktHeaderBitReader bin)
    {
        ...
        val |= ephArray[1];
        if (val != Markers.EPH)
            throw new ApplicationException("Corrupted Bitstream: Could not parse EPH " + "marker ! ");
    }
