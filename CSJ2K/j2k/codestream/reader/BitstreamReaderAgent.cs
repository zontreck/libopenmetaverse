...
public abstract class BitstreamReaderAgent : CodedCBlkDataSrcDec
{
    ...
    public virtual int getTileWidth(int rl)
    {
        ...
        if (rl > mindl)
            throw new ArgumentException("Requested resolution level" + " is not available for, at " +
                                        "least, one component in " + "tile: " + ctX + "x" + ctY);
        int ctulx, ntulx;
    ...
    /// </param>
    /// <returns>
    ///     The width in pixels of component <tt>c</tt> in tile <tt>t</tt>
    ...
    public int getTileCompWidth(int t, int c, int rl)
    {
        var tIdx = TileIdx;
        if (t != tIdx)
            throw new ApplicationException("Asking the tile-component width of a tile " +
    ...
    /// </param>
    /// <returns>
    ///     The height in pixels of component <tt>c</tt> in the current
    ///     tile.
    /// </returns>
    ...
    /// </param>
    /// <returns>
    ///     The width in pixels of component <tt>c</tt> in the overall
    ///     image.
    /// </returns>
    ...
    /// </param>
    /// <returns>
    ///     The height in pixels of component <tt>c</tt> in the overall
    ///     image.
    /// </returns>
    ...
    {
        if (t != TileIdx)
            throw new ArgumentException("Can not request subband" + " tree of a different tile" +
                                        " than the current one");
        if (c < 0 || c >= nc) throw new ArgumentException("Component index out of range");
        return subbTrees[c];
    }

    public abstract DecLyrdCBlk getCodeBlock(int param1, int param2, int param3, SubbandSyn param4, int param5,
        int param6, DecLyrdCBlk param7);
    ...
    /// </exception>
    public static BitstreamReaderAgent createInstance(RandomAccessIO in_Renamed, HeaderDecoder hd, ParameterList pl,
        DecoderSpecs decSpec, bool cdstrInfo, HeaderInfo hi)
    {
        // Check parameters
        ...
        if (!sb.isNode)
        {
            ...
            else
            {
                ...
                if (sb.ulcx - acb0x < 0 || sb.ulcy - acb0y < 0)
                    throw new ArgumentException("Invalid code-blocks " + "partition origin or " +
