...
public class FileBitstreamReaderAgent : BitstreamReaderAgent
{
    ...
    public FileBitstreamReaderAgent(HeaderDecoder hd, RandomAccessIO ehs, DecoderSpecs decSpec, ParameterList pl,
        bool cdstrInfo, HeaderInfo hi) : base(hd, decSpec)
    {
        this.pl = pl;
        printInfo = cdstrInfo;
        this.hi = hi;
        ...
        try
        {
            ...
            if (trate == -1) trate = float.MaxValue;
        }
        catch (FormatException)
        {
            throw new ApplicationException("Invalid value in 'rate' option: " + pl.getParameter("rate"));
        }
        catch (ArgumentException)
        {
            throw new ApplicationException("'rate' option is missing");
        }
        ...
        {
            throw new ApplicationException("Invalid value in 'nbytes' option: " + pl.getParameter("nbytes"));
        }
        catch (ArgumentException)
        {
            throw new ApplicationException("'nbytes' option is missing");
        }
        ...
        {
            throw new ApplicationException("Invalid value in 'ncb_quit' option: " + pl.getParameter("ncb_quit"));
        }
        catch (ArgumentException)
        {
            throw new ApplicationException("'ncb_quit' option is missing");
        }

        if (ncbQuit != -1 && !isTruncMode)
            throw new ApplicationException("Cannot use -parsing and -ncb_quit condition at " + "the same time.");
        ...
        {
            throw new ApplicationException("Invalid value in 'l_quit' option: " + pl.getParameter("l_quit"));
        }
        catch (ArgumentException)
        {
            throw new ApplicationException("'l_quit' option is missing");
        }
        ...
        if (anbytes > tnbytes) throw new ApplicationException("Requested bitrate is too small.");
        ...
        try
        {
            while (remainingTileParts != 0)
            {
                ...
                {
                    if (anbytes + tilePartHeadLen[t][tp] > tnbytes) break;
        ...
        catch (EndOfStreamException)
        {
            ...
            if (pl.getParameter("res") == null)
                ...
                try
                {
                    ...
                    if (targetRes < 0)
                        throw new ArgumentException("Specified negative " + "resolution level " + "index: " +
                                                    targetRes);
                }
                catch (FormatException)
                {
                    throw new ArgumentException("Invalid resolution level " + "index ('-res' option) " +
                                                pl.getParameter("res"));
                }
            ...
            {
                FacilityManager.getMsgLogger().printmsg(MsgLogger_Fields.WARNING,
                    "Specified resolution level (" + targetRes + ") is larger" +
                    " than the maximum value. Setting it to " + mdl + " (maximum value)");
                targetRes = mdl;
            }

            // Backup nBytes
            for (var tIdx = 0; tIdx < nt; tIdx++) baknBytes[tIdx] = nBytes[tIdx];
        ...
        if (!isEOCFound && !isPsotEqualsZero)
            try
            {
                ...
                if (!rateReached && !isPsotEqualsZero && eocCheck != Markers.EOC)
                    FacilityManager.getMsgLogger().printmsg(MsgLogger_Fields.WARNING,
        ...
        for (var tIdx = 0; tIdx < nt; tIdx++)
        {
            ...
            if (printInfo)
                FacilityManager.getMsgLogger()
                    .println("" + hi.toStringTileHeader(tIdx, tilePartLen[tIdx].Length), 2, 2);
        }
    }
    ...
    public virtual int getNumTileParts(int t)
    {
        if (firstPackOff == null || firstPackOff[t] == null)
            throw new ApplicationException("Tile " + t + " not found in input codestream.");
    ...
    private void allocateRate()
    {
        ...
        // error
        if (anbytes > stopOff) throw new ApplicationException("Requested bitrate is too small for parsing");
    ...
    private int readTilePartHeader()
    {
        ...
        ms.isot = tile;
        if (tile > 65534) throw new CorruptedCodestreamException("Tile index too high in " + "tile-part.");

        // Psot
        var psot = in_Renamed.readInt();
        ms.psot = psot;
        isPsotEqualsZero = psot != 0 ? false : true;
        if (psot < 0) throw new NotImplementedException("Tile length larger " + "than maximum supported");
        // TPsot
        ...
        else
        {
            ...
            else
            {
                ...
                if (tileParts[tile] != nrOfTileParts)
                {
                    ...
                    var tmpA = tilePartLen[tile];
                    tilePartLen[tile] = new int[nrOfTileParts];
                    for (var i = 0; i < tileParts[tile] - 1; i++) tilePartLen[tile][i] = tmpA[i];

                    // Increase and re-copy tilePartNum array                
                    tmpA = tilePartNum[tile];
                    tilePartNum[tile] = new int[nrOfTileParts];
                    for (var i = 0; i < tileParts[tile] - 1; i++) tilePartNum[tile][i] = tmpA[i];

                    // Increase and re-copy firstPackOff array
                    tmpA = firstPackOff[tile];
                    firstPackOff[tile] = new int[nrOfTileParts];
                    for (var i = 0; i < tileParts[tile] - 1; i++) firstPackOff[tile][i] = tmpA[i];

                    // Increase and re-copy tilePartHeadLen array
                    tmpA = tilePartHeadLen[tile];
                    tilePartHeadLen[tile] = new int[nrOfTileParts];
                    for (var i = 0; i < tileParts[tile] - 1; i++) tilePartHeadLen[tile][i] = tmpA[i];
                }
            }
        }
    ...
    private bool readLyResCompPos(int[][] lys, int lye, int ress, int rese, int comps, int compe)
    {
        ...
        int hlen, plen;
        var pph = false;
        if ((bool)decSpec.pphs.getTileDef(t)) pph = true;
        for (var l = minlys; l < lye; l++)
        {
            ...
            for (var c = comps; c < compe; c++)
            {
                ...
                for (var p = 0; p < nPrec; p++)
                {
                    ...
                    if (pph) pktDec.readPktHead(l, r, c, p, cbI[c][r], nBytes);
                    ...
                    if (!pph) status = pktDec.readPktHead(l, r, c, p, cbI[c][r], nBytes);
    ...
    private bool readResLyCompPos(int[][] lys, int lye, int ress, int rese, int comps, int compe)
    {
        ...
        for (var c = comps; c < compe; c++)
        {
            ...
            for (var r = ress; r < rese; r++)
            {
                ...
                if (r > mdl[c])
                    continue;
                if (lys[c] != null && r < lys[c].Length && lys[c][r] < minlys) minlys = lys[c][r];
            }
        }
    ...
    private bool readResPosCompLy(int[][] lys, int lye, int ress, int rese, int comps, int compe)
    {
        ...
        // components
        if (nPrec == 0) throw new ApplicationException("Image cannot have no precinct");
            ...
            for (var py = 0; py <= pyend; py++)
            {
                ...
                for (var px = 0; px <= pxend; px++)
                {
                    ...
                    for (var c = comps; c < compe; c++)
                    {
                        ...
                        if (r > mdl[c])
                            continue;
                        if (nextPrec[c][r] >= pktDec.getNumPrecinct(c, r)) continue;
                        ...
                        for (var l = minlys; l < lye; l++)
                        {
                            ...
                            // read header
                            if (pph) pktDec.readPktHead(l, r, c, nextPrec[c][r], cbI[c][r], nBytes);
                            ...
                            if (start > lastByte && curTilePart < firstPackOff[t].Length - 1)
                            {
                                curTilePart++;
                                in_Renamed.seek(firstPackOff[t][curTilePart]);
                                lastByte = in_Renamed.Pos + tilePartLen[t][curTilePart] - 1 -
                            ...
                            if (!pph) status = pktDec.readPktHead(l, r, c, nextPrec[c][r], cbI[c][r], nBytes);
    ...
    {
        pktHL = ArrayList.Synchronized(new ArrayList(10));
        layerStarts = new List<int>(5);
        ...
        for (var i = 0; i < nChg; i++) change[i] = new int[6];
        ...
        try
        {
            ...
            if ((isTruncMode && firstPackOff == null) || firstPackOff[t] == null) return;
        ...
        for (var c = 0; c < this.nc; c++) lys[c] = new int[(int)decSpec.dls.getTileCompVal(t, c) + 1];
        ...
        else if (nBytes[t] < totTileLen[t] - totTileHeadLen[t])
        {
            ...
            for (var i = pktHL.Count - 1; i >= 0; i--) pktHeadLen[i] = (int)pktHL[i];
            ...
            for (var l = 0; l < nl; l++)
            {
                ...
                for (var r = 0; r < mres; r++)
                {
                    ...
                    for (var s = 0; s < msub; s++)
                    {
                        ...
                        var mnby = 0;
                        for (var c = 0; c < nc; c++)
                            if (cbI[c] != null && cbI[c][r] != null && cbI[c][r][s] != null &&
                        ...
                        for (var m = 0; m < mnby; m++)
                        {
                            ...
                            for (var c = 0; c < nc; c++)
                                if (cbI[c] != null && cbI[c][r] != null && cbI[c][r][s] != null &&
                                    cbI[c][r][s][m] != null && cbI[c][r][s][m].Length > mnbx)
                                    mnbx = cbI[c][r][s][m].Length;
                            for (var n = 0; n < mnbx; n++)
                            for (var c = 0; c < nc; c++)
                            {
                                if (cbI[c] == null || cbI[c][r] == null || cbI[c][r][s] == null ||
                                    cbI[c][r][s][m] == null || cbI[c][r][s][m][n] == null) continue;
                                cb = cbI[c][r][s][m][n];
        ...
        else
        {
            ...
            if (t < getNumTiles() - 1) nBytes[t + 1] += nBytes[t] - (totTileLen[t] - totTileHeadLen[t]);
        }
    }
    ...
    public override void setTile(int x, int y)
    {
        ...
        if (x < 0 || y < 0 || x >= ntX || y >= ntY) throw new ArgumentException();
        var t = y * ntX + x;
        ...
        {
            anbytes = headLen;
            if (!isTruncMode) anbytes += 2;
            // Restore values of nBytes
            for (var tIdx = 0; tIdx < nt; tIdx++) nBytes[tIdx] = baknBytes[tIdx];
        }
        ...
        catch (IOException e)
        {
            ...
            throw new ApplicationException("IO Error when reading tile " + x + " x " + y);
        }
    }
    ...
    public override DecLyrdCBlk getCodeBlock(int c, int m, int n, SubbandSyn sb, int fl, int nl, DecLyrdCBlk ccb)
    {
        ...
        if (r > targetRes + maxdl - decSpec.dls.Min)
            throw new ApplicationException(
                "JJ2000 error: requesting a code-block " + "disallowed by the '-res' option.");
        ...
        try
        {
            ...
            if (fl < 1 || fl > numLayers || fl + nl - 1 > numLayers) throw new ArgumentException();
        }
        catch (IndexOutOfRangeException)
        {
            throw new ArgumentException("Code-block (t:" + t + ", c:" + c + ", r:" + r + ", s:" + s + ", " + m + "x" +
        ...
        if (ccb == null) ccb = new DecLyrdCBlk();
        ccb.m = m;
        ccb.n = n;
        ccb.nl = 0;
        ccb.dl = 0;
        ...
        else if ((options & StdEntropyCoderOptions.OPT_BYPASS) != 0)
        {
            ...
            else
            {
                ...
                for (tpidx = ccb.ftpIdx; tpidx < ccb.nTrunc; tpidx++)
                    ...
                    {
                        passtype = (tpidx + StdEntropyCoderOptions.NUM_EMPTY_PASSES_IN_MS_BP) %
        ...
        // ccb.data creation
        if (ccb.data == null || ccb.data.Length < ccb.dl) ccb.data = new byte[ccb.dl];

        // ccb.tsLengths creation
        if (nts > 1 && (ccb.tsLengths == null || ccb.tsLengths.Length < nts))
            ccb.tsLengths = new int[nts];
        else if (nts > 1 && (options & (StdEntropyCoderOptions.OPT_BYPASS | StdEntropyCoderOptions.OPT_TERM_PASS)) ==
                 StdEntropyCoderOptions.OPT_BYPASS) ArrayUtil.intArraySet(ccb.tsLengths, 0);
        ...
        if (nts == 1 && ccb.tsLengths != null) ccb.tsLengths[0] = ccb.dl;
