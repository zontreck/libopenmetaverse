...
public class HeaderDecoder
{
    ...
    /// </exception>
    /// <exception cref="CorruptedCodestreamException">
    ...
    public virtual int TileOfTileParts
    {
        set
        {
            if (nPPMMarkSeg != 0) tileOfTileParts.Add(value);
        }
    }
    ...
    private SynWTFilter readFilter(BinaryReader ehs, int[] filtIdx)
    {
        ...
        if (kid >= 1 << 7) throw new NotImplementedException("Custom filters not supported");
        ...
        switch (kid)
        {
            ...
            default:
                throw new CorruptedCodestreamException("Specified wavelet filter " + "not" + " JPEG 2000 part I " +
                                                       "compliant");
        }
    }
    ...
    public virtual void checkMarkerLength(BinaryReader ehs, string str)
    {
        ...
        if ((int)available != 0)
            FacilityManager.getMsgLogger()
                .printmsg(MsgLogger_Fields.WARNING, str + " length was short, attempting to resync.");
    }
    ...
    private void readSIZ(BinaryReader ehs)
    {
        ...
        if (ms.rsiz > 2) throw new ApplicationException("Codestream capabiities not JPEG 2000 - Part I" + " compliant");
    ...
    private void readCOM(BinaryReader ehs, bool mainh, int tileIdx, int comIdx)
    {
        ...
        switch (ms.rcom)
        {
            ...
            case Markers.RCOM_LATIN:
                ...
                for (var i = 0; i < ms.lcom - 4; i++) ms.ccom[i] = ehs.ReadByte();
                break;
            default:
                // --- Unknown or unsupported markers ---
                // (skip them and see if we can get way with it)
                FacilityManager.getMsgLogger().printmsg(MsgLogger_Fields.WARNING,
                    "COM marker registered as " + ms.rcom + " unknown, ignoring (this might crash the " +
                ...
                temp_Int64 = temp_BinaryReader.BaseStream.Seek(ms.lcom - 4, SeekOrigin.Current) - temp_Int64;
                // CONVERSION PROBLEM?
    ...
    private void readQCD(BinaryReader ehs, bool mainh, int tileIdx, int tpIdx)
    {
        ...
        if (qType == Markers.SQCX_NO_QUANTIZATION)
        {
            ...
            for (var i2 = 0; i2 < maxrl + 1; i2++) tmpArray[i2] = new int[4];
            ms.spqcd = tmpArray;
        ...
        {
            var maxrl = qType == Markers.SQCX_SCALAR_DERIVED ? 0 :
                mainh ? (int)decSpec.dls.getDefault() : (int)decSpec.dls.getTileDef(tileIdx);
            int j, rl; // i removed
            int minb, maxb, hpd;
            int tmp;
            ...
            for (var i3 = 0; i3 < maxrl + 1; i3++) tmpArray2[i3] = new int[4];
            ms.spqcd = tmpArray2;
    ...
    private void readQCC(BinaryReader ehs, bool mainh, int tileIdx, int tpIdx)
    {
        ...
        if (cComp >= nComp) throw new CorruptedCodestreamException("Invalid component " + "index in QCC marker");
        ...
        {
            var maxrl = qType == Markers.SQCX_SCALAR_DERIVED ? 0 :
                mainh ? (int)decSpec.dls.getCompDef(cComp) : (int)decSpec.dls.getTileCompVal(tileIdx, cComp);
            int j, rl; // i removed
            int minb, maxb, hpd;
    ...
    private void readCOD(BinaryReader ehs, bool mainh, int tileIdx, int tpIdx)
    {
        ...
        {
            if (cb0x != -1 && cb0x == 0)
                throw new ArgumentException("Code-block partition " + "origin redefined in new" +
        ...
        if ((ecOptions & ~ (StdEntropyCoderOptions.OPT_BYPASS | StdEntropyCoderOptions.OPT_RESET_MQ |
                            StdEntropyCoderOptions.OPT_TERM_PASS | StdEntropyCoderOptions.OPT_VERT_STR_CAUSAL |
                            StdEntropyCoderOptions.OPT_PRED_TERM | StdEntropyCoderOptions.OPT_SEG_SYMBOLS)) != 0)
            throw new CorruptedCodestreamException("Unknown \"code-block " + "style\" in SPcod field, " +
                                                   "COD marker: 0x" + Convert.ToString(ecOptions, 16));
        ...
        var v = new ArrayList[2];
        v[0] = ArrayList.Synchronized(new ArrayList(10));
        v[1] = ArrayList.Synchronized(new ArrayList(10));
    ...
    private void readCOC(BinaryReader ehs, bool mainh, int tileIdx, int tpIdx)
    {
        ...
        if ((ecOptions & ~ (StdEntropyCoderOptions.OPT_BYPASS | StdEntropyCoderOptions.OPT_RESET_MQ |
            ...
            throw new CorruptedCodestreamException("Unknown \"code-block " + "context\" in SPcoc field, " +
                                                   "COC marker: 0x" + Convert.ToString(ecOptions, 16));
    ...
    {
        var useShort = nComp >= 256 ? true : false;
        int tmp;
        var nOldChg = 0;
        HeaderInfo.POC ms;
        ...
        if (nOldChg != 0)
        {
            ...
            for (var i = 0; i < ntotChg; i++) tmpArray[i] = new int[6];
            change = tmpArray;
        ...
        else
        {
            ...
            for (var i2 = 0; i2 < newChg; i2++) tmpArray2[i2] = new int[6];
            change = tmpArray2;
    ...
    private void readTLM(BinaryReader ehs)
    {
        ...
        temp_BinaryReader = ehs;
        temp_Int64 = temp_BinaryReader.BaseStream.Position;
        temp_Int64 = temp_BinaryReader.BaseStream.Seek(length - 2, SeekOrigin.Current) - temp_Int64;
        // CONVERSION PROBLEM?
    ...
    private void readRGN(BinaryReader ehs, bool mainh, int tileIdx, int tpIdx)
    {
        ...
        if (comp >= nComp) throw new CorruptedCodestreamException("Invalid component " + "index in RGN marker" + comp);
    ...
    private void readPPM(BinaryReader ehs)
    {
        ...
        if (pPMMarkerData == null)
        {
            ...
            tileOfTileParts = ArrayList.Synchronized(new ArrayList(10));
        ...
        ehs.BaseStream.Read(pPMMarkerData[indx], 0,
            remSegLen); //SupportClass.ReadInput(ehs.BaseStream, pPMMarkerData[indx], 0, remSegLen);
    ...
    private void readPPT(BinaryReader ehs, int tile, int tpIdx)
    {
        ...
        if (tilePartPkdPktHeaders == null) tilePartPkdPktHeaders = new byte[nTiles][][][];

        if (tilePartPkdPktHeaders[tile] == null) tilePartPkdPktHeaders[tile] = new byte[nTileParts[tile]][][];
        ...
        if (nfMarkSeg == 0)
            ...
            if (marker != Markers.SIZ)
                throw new CorruptedCodestreamException("First marker after " + "SOC " + "must be SIZ " +
                                                       Convert.ToString(marker, 16));

        var htKey = ""; // Name used as a key for the hash-table
        if (ht == null) ht = Hashtable.Synchronized(new Hashtable());

        switch (marker)
        {
            case Markers.SIZ:
                if ((nfMarkSeg & SIZ_FOUND) != 0)
                    throw new CorruptedCodestreamException("More than one SIZ marker " + "segment found in main " +
                                                           "header");
                nfMarkSeg |= SIZ_FOUND;
                htKey = "SIZ";
                break;
    ...
    public virtual void extractTilePartMarkSeg(short marker, RandomAccessIO ehs, int tileIdx, int tilePartIdx)
    {
        ...
        switch (marker)
        {
            ...
            case Markers.PPT:
                nfMarkSeg |= PPT_FOUND;
                if (nPPTMarkSeg == null) nPPTMarkSeg = new int[nTiles][];
                if (nPPTMarkSeg[tileIdx] == null) nPPTMarkSeg[tileIdx] = new int[nTileParts[tileIdx]];
            ...
            case Markers.PLT:
                if ((nfMarkSeg & PLM_FOUND) != 0)
                    throw new CorruptedCodestreamException("PLT marker found even" + "though PLM marker " +
            ...
            default:
                htKey = "UNKNOWN";
                FacilityManager.getMsgLogger().printmsg(MsgLogger_Fields.WARNING,
                    "Non recognized marker segment (0x" + Convert.ToString(marker, 16) + ") in tile-part header" +
                    " of tile " + tileIdx + " !");
                break;
        }
    ...
    private void readFoundMainMarkSeg()
    {
        ...
        if ((nfMarkSeg & SIZ_FOUND) != 0)
        {
            bais = new MemoryStream((byte[])ht["SIZ"]);
            readSIZ(new EndianBinaryReader(bais, true));
            bais.Dispose();
        }
        ...
        if ((nfMarkSeg & COM_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["COM" + i]);
                readCOM(new EndianBinaryReader(bais, true), true, 0, i);
                bais.Dispose();
            }
        ...
        if ((nfMarkSeg & CRG_FOUND) != 0)
        {
            bais = new MemoryStream((byte[])ht["CRG"]);
            readCRG(new EndianBinaryReader(bais, true));
            bais.Dispose();
        }
        ...
        if ((nfMarkSeg & COD_FOUND) != 0)
        {
            bais = new MemoryStream((byte[])ht["COD"]);
            readCOD(new EndianBinaryReader(bais, true), true, 0, 0);
            bais.Dispose();
        }
        ...
        if ((nfMarkSeg & COC_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["COC" + i]);
                readCOC(new EndianBinaryReader(bais, true), true, 0, 0);
                bais.Dispose();
            }
        ...
        if ((nfMarkSeg & RGN_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["RGN" + i]);
                readRGN(new EndianBinaryReader(bais, true), true, 0, 0);
                bais.Dispose();
            }
        ...
        if ((nfMarkSeg & QCD_FOUND) != 0)
        {
            bais = new MemoryStream((byte[])ht["QCD"]);
            readQCD(new EndianBinaryReader(bais, true), true, 0, 0);
            bais.Dispose();
        }
        ...
        if ((nfMarkSeg & QCC_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["QCC" + i]);
                readQCC(new EndianBinaryReader(bais, true), true, 0, 0);
                bais.Dispose();
            }
        ...
        if ((nfMarkSeg & POC_FOUND) != 0)
        {
            bais = new MemoryStream((byte[])ht["POC"]);
            readPOC(new EndianBinaryReader(bais, true), true, 0, 0);
            bais.Dispose();
        }
        ...
        if ((nfMarkSeg & PPM_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["PPM" + i]);
                readPPM(new EndianBinaryReader(bais));
                bais.Dispose();
            }
    ...
    public virtual void readFoundTilePartMarkSeg(int tileIdx, int tpIdx)
    {
        ...
        {
            bais = new MemoryStream((byte[])ht["COD"]);
            readCOD(new EndianBinaryReader(bais, true), false, tileIdx, tpIdx);
        }
        ...
        if ((nfMarkSeg & COC_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["COC" + i]);
                readCOC(new EndianBinaryReader(bais, true), false, tileIdx, tpIdx);
            }
        ...
        if ((nfMarkSeg & RGN_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["RGN" + i]);
                readRGN(new EndianBinaryReader(bais, true), false, tileIdx, tpIdx);
            }
        ...
        {
            bais = new MemoryStream((byte[])ht["QCD"]);
            readQCD(new EndianBinaryReader(bais, true), false, tileIdx, tpIdx);
        }
        ...
        if ((nfMarkSeg & QCC_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["QCC" + i]);
                readQCC(new EndianBinaryReader(bais, true), false, tileIdx, tpIdx);
            }
        ...
        {
            bais = new MemoryStream((byte[])ht["POC"]);
            readPOC(new EndianBinaryReader(bais, true), false, tileIdx, tpIdx);
        }
        ...
        if ((nfMarkSeg & COM_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["COM" + i]);
                readCOM(new EndianBinaryReader(bais, true), false, tileIdx, i);
            }
        ...
        if ((nfMarkSeg & PPT_FOUND) != 0)
            ...
            {
                bais = new MemoryStream((byte[])ht["PPT" + i]);
                readPPT(new EndianBinaryReader(bais, true), tileIdx, tpIdx);
            }
    ...
    public virtual MemoryStream getPackedPktHead(int tile)
    {
        if (pkdPktHeaders == null)
        {
            ...
            for (i = nTiles - 1; i >= 0; i--) pkdPktHeaders[i] = new MemoryStream();
            if (nPPMMarkSeg != 0)
            {
                ...
                byte[] temp;
                MemoryStream pph;
                var allNppmIppm = new MemoryStream();
