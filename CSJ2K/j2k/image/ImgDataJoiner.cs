...
public class ImgDataJoiner : BlkImgDataSrc
{
    ...
    ///     <br>
    ///         <u>Example:</u> Reading R,G,B components from 3 PGM files.
    ///         <br>
    ///             <tt>
    ...
    public ImgDataJoiner(BlkImgDataSrc[] imD, int[] cIdx)
    {
        ...
        for (i = 0; i < nc; i++)
            if (imD[i].getNumTiles() != 1 || imD[i].getCompULX(cIdx[i]) != 0 || imD[i].getCompULY(cIdx[i]) != 0)
                throw new ArgumentException("All input components must, " + "not use tiles and must " + "have " +
                                            "the origin at the canvas " + "origin");
        ...
        for (i = 0; i < nc; i++)
        {
            ...
            if ((maxW + subsX[i] - 1) / subsX[i] != imD[i].getCompImgWidth(cIdx[i]) ||
                ...
                throw new ApplicationException("Can not compute component subsampling " +
    ...
    /// </param>
    /// <returns>
    ///     The height in pixels of component <tt>n</tt> in the overall
    ///     image.
    /// </returns>
    ...
    ///     <i>
    ///         b</b> then for unsigned data the
