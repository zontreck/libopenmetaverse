...
public class ImgReaderPGM : ImgReader
{
    ...
    /// </exception>
    public ImgReaderPGM(FileInfo file) : this(SupportClass.RandomAccessFileSupport.CreateRandomAccessFile(file, "r"))
    {
    }
    ...
    /// </exception>
    //UPGRADE_TODO: Class 'java.io.RandomAccessFile' was converted to 'System.IO.FileStream' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javaioRandomAccessFile'"
    public ImgReaderPGM(FileStream in_Renamed)
    {
    ...
    public override DataBlk getInternCompData(DataBlk blk, int c)
    {
        ...
        // Check line buffer
        if (buf == null || buf.Length < blk.w) buf = new byte[blk.w];
    ...
    private void confirmFileType()
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            if (b != type[i])
            {
                if (i == 1 && b == 50)
                    //i.e 'P2'
                    throw new ArgumentException("JJ2000 does not support" + " ascii-PGM files. Use " +
