...
public class ImgReaderPPM : ImgReader
{
    ...
    /// </param>
    public ImgReaderPPM(FileInfo file) : this(SupportClass.RandomAccessFileSupport.CreateRandomAccessFile(file, "r"))
    {
    }
    ...
    ///     If an error occurs while opening the file.
    /// </param>
    public ImgReaderPPM(string fname) : this(SupportClass.RandomAccessFileSupport.CreateRandomAccessFile(fname, "r"))
    {
    }
    ...
    /// </exception>
    //UPGRADE_TODO: Class 'java.io.RandomAccessFile' was converted to 'System.IO.FileStream' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javaioRandomAccessFile'"
    private ImgReaderPPM(FileStream in_Renamed)
    {
    ...
    public override DataBlk getInternCompData(DataBlk blk, int c)
    {
        ...
        // components
        if (barr[c] == null || dbi.ulx > blk.ulx || dbi.uly > blk.uly || dbi.ulx + dbi.w < blk.ulx + blk.w ||
            dbi.uly + dbi.h < blk.uly + blk.h)
        {
            ...
            if (barr[c] == null || barr[c].Length < blk.w * blk.h) barr[c] = new int[blk.w * blk.h];
            blk.Data = barr[c];

            i = (c + 1) % 3;
            if (barr[i] == null || barr[i].Length < blk.w * blk.h) barr[i] = new int[blk.w * blk.h];
            i = (c + 2) % 3;
            if (barr[i] == null || barr[i].Length < blk.w * blk.h) barr[i] = new int[blk.w * blk.h];
            ...
            // Check line buffer
            if (buf == null || buf.Length < 3 * blk.w) buf = new byte[3 * blk.w];
            ...
            try
            {
                ...
                for (i = blk.uly; i < mi; i++)
                {
                    ...
                    // header offset
                    in_Renamed.Seek(offset + i * 3 * w + 3 * blk.ulx, SeekOrigin.Begin);
    ...
    public override DataBlk getCompData(DataBlk blk, int c)
    {
        ...
        // Copy the data
        if (bakarr == null) bakarr = new int[w * h];
    ...
    private void confirmFileType()
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            if (b != type[i])
            {
                if (i == 1 && b == 51)
                    // i.e 'P3'
                    throw new ArgumentException("JJ2000 does not support" + " ascii-PPM files. Use " +
    ...
    private void skipCommentAndWhiteSpace()
    {
        ...
        offset--;
        in_Renamed.Seek(offset, SeekOrigin.Begin);
    }
