...
///             <ul>
///                 <li>
///                     <i>ws</i> (white-spaces) is any combination of characters ' ' and
///                     '\t'.
///                 </li>
///                 <li>
///                     <i>endianess</i> equals "LM" or "ML"(resp. little-endian or
///                     big-endian)
///                 </li>
///                 <li>
///                     <i>sign</i> equals "+" or "-" (resp. unsigned or signed). If omited,
///                     values are supposed to be unsigned.
///                 </li>
///                 <li><i>bit-depth</i> that can be any number between 1 and 31.</li>
///                 <li>
///                     <i>width</i> and <i>height</i> are the image dimensions (in
///                     pixels).
///                 </li>
///             </ul>
...
public class ImgReaderPGX : ImgReader, EndianType
{
    ...
    /// </exception>
    public ImgReaderPGX(FileInfo in_Renamed)
    {
        string header;

        // Check if specified file exists
        bool tmpBool;
        if (File.Exists(in_Renamed.FullName))
            tmpBool = true;
        else
            tmpBool = Directory.Exists(in_Renamed.FullName);
        if (!tmpBool) throw new ArgumentException("PGX file " + in_Renamed.Name + " does not exist");

        //Opens the given file
        this.in_Renamed = SupportClass.RandomAccessFileSupport.CreateRandomAccessFile(in_Renamed, "r");
        try
        {
            var in_reader = new StreamReader(this.in_Renamed);
        ...
        {
            throw new IOException(in_Renamed.Name + " is not a PGX file");
        }

        if (header == null) throw new IOException(in_Renamed.Name + " is an empty file");
        ...
        try
        {
            ...
            try
            {
                ...
                if (bitDepth <= 0 || bitDepth > 31)
                    throw new IOException(in_Renamed.Name + " is not a valid PGX file");
    ...
    ///     The input file name.
    /// </param>
    public ImgReaderPGX(string inName) : this(new FileInfo(inName))
    {
    }
    ...
    ///         If this number is
    ///         <i>
    ///             b</b> then the nominal range is between
    ...
    public override DataBlk getInternCompData(DataBlk blk, int c)
    {
        ...
        if (buf == null || buf.Length < packBytes * blk.w) buf = new byte[packBytes * blk.w];
        try
        {
            switch (packBytes)
            {
                ...
                case 1: // Samples packed into 1 byte
                    ...
                    if (isSigned)
                        ...
                        {
                            // Reposition in input
                            in_Renamed.Seek(offset + i * w + blk.ulx, SeekOrigin.Begin);
                        ...
                        for (i = blk.uly; i < mi; i++)
                        {
                            ...
                            in_Renamed.Read(buf, 0, blk.w);
                            for (k = (i - blk.uly) * blk.w + blk.w - 1, j = blk.w - 1; j >= 0; k--)
                                barr[k] = SupportClass.URShift((buf[j--] & 0xFF) << paddingLength, paddingLength) -
                                          levShift;
                        }
                ...
                case 2: // Samples packed into 2 bytes
                    ...
                    if (isSigned)
                        ...
                        {
                            // Reposition in input
                            in_Renamed.Seek(offset + 2 * (i * w + blk.ulx), SeekOrigin.Begin);
                            in_Renamed.Read(buf, 0, blk.w << 1);
                            switch (byteOrder)
                            {
                                case EndianType_Fields.LITTLE_ENDIAN:
                                    for (k = (i - blk.uly) * blk.w + blk.w - 1, j = (blk.w << 1) - 1; j >= 0; k--)
                                        barr[k] = ((((buf[j--] & 0xFF) << 8) | (buf[j--] & 0xFF)) << paddingLength) >>
                                                  paddingLength;
                                    break;

                                case EndianType_Fields.BIG_ENDIAN:
                                    for (k = (i - blk.uly) * blk.w + blk.w - 1, j = (blk.w << 1) - 1; j >= 0; k--)
                                        barr[k] = (((buf[j--] & 0xFF) | ((buf[j--] & 0xFF) << 8)) << paddingLength) >>
                                                  paddingLength;
                                    break;

                                default:
                                    throw new ApplicationException("Internal JJ2000 bug");
                            }
                        }
                    else
                        // If not signed
                        for (i = blk.uly; i < mi; i++)
                        {
                            ...
                            switch (byteOrder)
                            {
                                case EndianType_Fields.LITTLE_ENDIAN:
                                    for (k = (i - blk.uly) * blk.w + blk.w - 1, j = (blk.w << 1) - 1; j >= 0; k--)
                                        barr[k] = SupportClass.URShift(
                                            (((buf[j--] & 0xFF) << 8) | (buf[j--] & 0xFF)) << paddingLength,
                                ...
                                case EndianType_Fields.BIG_ENDIAN:
                                    for (k = (i - blk.uly) * blk.w + blk.w - 1, j = (blk.w << 1) - 1; j >= 0; k--)
                                        barr[k] = SupportClass.URShift(
                                            ((buf[j--] & 0xFF) | ((buf[j--] & 0xFF) << 8)) << paddingLength,
                                            paddingLength) - levShift;
                                    break;
                ...
                case 4: // Samples packed into 4 bytes
                    ...
                    if (isSigned)
                        ...
                        {
                            // Reposition in input
                            in_Renamed.Seek(offset + 4 * (i * w + blk.ulx), SeekOrigin.Begin);
                            in_Renamed.Read(buf, 0, blk.w << 2);
                            switch (byteOrder)
                            {
                                case EndianType_Fields.LITTLE_ENDIAN:
                                    for (k = (i - blk.uly) * blk.w + blk.w - 1, j = (blk.w << 2) - 1; j >= 0; k--)
                                        barr[k] = ((((buf[j--] & 0xFF) << 24) | ((buf[j--] & 0xFF) << 16) |
                                                    ((buf[j--] & 0xFF) << 8) | (buf[j--] & 0xFF)) << paddingLength) >>
                                                  paddingLength;
                                    break;
                                ...
                                case EndianType_Fields.BIG_ENDIAN:
                                    for (k = (i - blk.uly) * blk.w + blk.w - 1, j = (blk.w << 2) - 1; j >= 0; k--)
                                        barr[k] = (((buf[j--] & 0xFF) | ((buf[j--] & 0xFF) << 8) |
                                                    ((buf[j--] & 0xFF) << 16) | ((buf[j--] & 0xFF) << 24)) <<
                        ...
                        for (i = blk.uly; i < mi; i++)
                        {
                            ...
                            switch (byteOrder)
                            {
                                ...
                                case EndianType_Fields.BIG_ENDIAN:
                                    for (k = (i - blk.uly) * blk.w + blk.w - 1, j = (blk.w << 2) - 1; j >= 0; k--)
                                        barr[k] = SupportClass.URShift(
                                            ((buf[j--] & 0xFF) | ((buf[j--] & 0xFF) << 8) | ((buf[j--] & 0xFF) << 16) |
                                             ((buf[j--] & 0xFF) << 24)) << paddingLength, paddingLength) - levShift;
                                    break;
    ...
    public override string ToString()
    {
        ...
        return "ImgReaderPGX: WxH = " + w + "x" + h + ", Component = 0" + ", Bit-depth = " + bitDepth + ", signed = " +
