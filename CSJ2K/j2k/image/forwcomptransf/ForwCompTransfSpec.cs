        ...
        if (param == null)
        {
...

            {
                ...
                else
                {
                    ...
                    if (anfilt[0][0].FilterType == FilterTypes_Fields.W9X7)
                        setDefault("ict");
                    else
                        setDefault("rct");
                }
        ...
        while (stk.HasMoreTokens())
        {
            ...
            switch (word[0])
            {
                ...
                case 'c': // Components specification
                    throw new ArgumentException("Component specific " + " parameters" + " not allowed with " +
                                                "'-Mct' option");
                ...
                default:
                    ...
                    {
                        if (nc < 3)
                            throw new ArgumentException("Cannot use component" + " transformation on a " +
                        ...
                        if (curSpecType == SPEC_DEF)
                            ...
                            for (var i = tileSpec.Length - 1; i >= 0; i--)
                                if (tileSpec[i])
                                {
                                    if (getFilterType(i, wfs) == FilterTypes_Fields.W5X3)
                                        setTileDef(i, "rct");
                                    else
                                        setTileDef(i, "ict");
                                }
	...
	private int getFilterType(int t, AnWTFilterSpec wfs)
    {
        ...
        for (var c = 1; c < nComp; c++)
            if (filtType[c] != filtType[0])
                reject = true;
        if (reject)
            throw new ArgumentException("Can not use component" + " transformation when " + "components do not use " +
                                        "the same filters");
        return filtType[0];
    }
}