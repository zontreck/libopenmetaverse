...
public class ForwCompTransf : ImgDataAdapter, BlkImgDataSrc
{
    ...
    public virtual DataBlk getInternCompData(DataBlk blk, int c)
    {
        switch (transfType)
        {
            ...
            default:
                throw new ArgumentException("Non JPEG 2000 part 1 " + "component" + " transformation for tile: " +
                                            tIdx);
        }
    }
    ...
    {
        if (ntdepth.Length < 3 && ttype != NONE) throw new ArgumentException();

        if (tdepth == null) tdepth = new int[ntdepth.Length];
        ...
        switch (ttype)
        {
            ...
            case FORW_RCT:
                if (ntdepth.Length > 3) Array.Copy(ntdepth, 3, tdepth, 3, ntdepth.Length - 3);
                // The formulas are:
    ...
    private void initForwRCT()
    {
        ...
        if (src.NumComps < 3) throw new ArgumentException();
        ...
        for (i = utd.Length - 1; i >= 0; i--) utd[i] = src.getNomRangeBits(i);
    ...
    private DataBlk forwRCT(DataBlk blk, int c)
    {
        ...
        if (c >= 0 && c <= 2)
        {
            ...
            {
                if (outBlk == null || outBlk.DataType != DataBlk.TYPE_INT) outBlk = new DataBlkInt();
                outBlk.w = w;
                outBlk.h = h;
                outBlk.ulx = blk.ulx;
                outBlk.uly = blk.uly;
    ...
    private DataBlk forwICT(DataBlk blk, int c)
    {
        ...
        {
            if (outBlk == null || outBlk.DataType != DataBlk.TYPE_FLOAT) outBlk = new DataBlkFloat();
            outBlk.w = w;
            outBlk.h = h;
            outBlk.ulx = blk.ulx;
            outBlk.uly = blk.uly;
        ...
        if (c >= 0 && c <= 2)
        {
            ...
            if (block0 == null) block0 = new DataBlkInt();
            if (block1 == null) block1 = new DataBlkInt();
            if (block2 == null) block2 = new DataBlkInt();
            block0.w = block1.w = block2.w = blk.w;
            block0.h = block1.h = block2.h = blk.h;
            block0.ulx = block1.ulx = block2.ulx = blk.ulx;
            block0.uly = block1.uly = block2.uly = blk.uly;
