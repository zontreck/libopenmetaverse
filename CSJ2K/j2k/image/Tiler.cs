...
public class Tiler : ImgDataAdapter, BlkImgDataSrc
{
    ...
    public Tiler(BlkImgDataSrc src, int ax, int ay, int px, int py, int nw, int nh) : base(src)
    {
        ...
        if (src.getNumTiles() != 1) throw new ArgumentException("Source is tiled");
        // Verify that source is not "canvased"
        if (src.ImgULX != 0 || src.ImgULY != 0) throw new ArgumentException("Source is \"canvased\"");
        ...
        if (x0siz - xt0siz >= xtsiz) xt0siz += (x0siz - xt0siz) / xtsiz * xtsiz;
        if (y0siz - yt0siz >= ytsiz) yt0siz += (y0siz - yt0siz) / ytsiz * ytsiz;
        if (x0siz - xt0siz >= xtsiz || y0siz - yt0siz >= ytsiz)
            FacilityManager.getMsgLogger().printmsg(MsgLogger_Fields.INFO,
                "Automatically adjusted tiling " + "origin to equivalent one (" + xt0siz + "," + yt0siz + ") so that " +
    ...
    {
        if (t != TileIdx)
            throw new ApplicationException("Asking the width of a tile-component which is " +
    ...
    {
        // Check tile indexes
        if (x < 0 || y < 0 || x >= ntX || y >= ntY) throw new ArgumentException("Tile's indexes out of bounds");
