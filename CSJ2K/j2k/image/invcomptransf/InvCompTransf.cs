...
public class InvCompTransf : ImgDataAdapter, BlkImgDataSrc
{
    ...
    ///     transform
    /// </summary>
    private readonly bool noCompTransf;
    ...
    public override void setTile(int x, int y)
    {
        ...
        // initializations
        if ((int)cts.getTileDef(tIdx) == NONE)
        {
            transfType = NONE;
        }
        else
        {
            ...
            var rev = 0;
            for (var c = 0; c < nc; c++) rev += wfs.isReversible(tIdx, c) ? 1 : 0;
            if (rev == 3)
                ...
                // Error
                throw new ArgumentException("Wavelet transformation and " + "component transformation" +
    ...
    {
        if (utdepth.Length < 3 && ttype != NONE) throw new ArgumentException();

        if (tdepth == null) tdepth = new int[utdepth.Length];
        ...
        switch (ttype)
        {
            ...
            case INV_RCT:
                if (utdepth.Length > 3) Array.Copy(utdepth, 3, tdepth, 3, utdepth.Length - 3);
                // The formulas are:
    ...
    private DataBlk invRCT(DataBlk blk, int c)
    {
        ...
        if (outdata[c] == null || dbi.ulx > blk.ulx || dbi.uly > blk.uly || dbi.ulx + dbi.w < blk.ulx + blk.w ||
    ...
    private DataBlk invICT(DataBlk blk, int c)
    {
        ...
        // If asking a component for the first time for this block,
        // do transform for the 3 components
        else if (outdata[c] == null || dbi.ulx > blk.ulx || dbi.uly > blk.uly || dbi.ulx + dbi.w < blk.ulx + blk.w ||
                 dbi.uly + dbi.h < blk.uly + blk.h)
        {
            int k, k0, k1, k2, mink, i;
            var w = blk.w; //width of output block
