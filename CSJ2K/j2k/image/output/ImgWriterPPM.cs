...
public class ImgWriterPPM : ImgWriter
{
    ...
    /// </seealso>
    public ImgWriterPPM(FileInfo out_Renamed, BlkImgDataSrc imgSrc, int n1, int n2, int n3)
    {
        // Check that imgSrc is of the correct type
        // Check that the component index is valid
        if (n1 < 0 || n1 >= imgSrc.NumComps || n2 < 0 || n2 >= imgSrc.NumComps || n3 < 0 || n3 >= imgSrc.NumComps ||
            imgSrc.getNomRangeBits(n1) > 8 || imgSrc.getNomRangeBits(n2) > 8 ||
            imgSrc.getNomRangeBits(n3) > 8) throw new ArgumentException("Invalid component indexes");
        // Initialize
    ...
    /// <seealso cref="DataBlk">
    /// </seealso>
    public ImgWriterPPM(string fname, BlkImgDataSrc imgSrc, int n1, int n2, int n3) : this(new FileInfo(fname), imgSrc,
        n1, n2, n3)
    {
    }
    ...
    public override void close()
    {
        ...
        if (out_Renamed.Length != 3 * w * h + offset)
        {
            ...
            for (i = 3 * w * h + offset - (int)out_Renamed.Length; i > 0; i--) out_Renamed.WriteByte(0);
        }
    ...
    public override void write(int ulx, int uly, int w, int h)
    {
        ...
        if (buf == null || buf.Length < 3 * w) buf = new byte[3 * w];
        ...
        for (i = 0; i < h; i++)
        {
            ...
            out_Renamed.Seek(offset + 3 * (this.w * (uly + tOffy + i) + ulx + tOffx), SeekOrigin.Begin);
    ...
    private void writeHeaderInfo()
    {
        ...
        // write 'P6' to file
        out_Renamed.Seek(0, SeekOrigin.Begin);
        ...
        byteVals = Encoding.ASCII.GetBytes(val);
