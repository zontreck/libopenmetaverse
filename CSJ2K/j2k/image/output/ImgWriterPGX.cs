...
public class ImgWriterPGX : ImgWriter
{
    ...
    /// </seealso>
    public ImgWriterPGX(FileInfo out_Renamed, BlkImgDataSrc imgSrc, int c, bool isSigned)
    {
        //Initialize
        this.c = c;
        bool tmpBool;
        if (File.Exists(out_Renamed.FullName))
            tmpBool = true;
        else
            tmpBool = Directory.Exists(out_Renamed.FullName);
        bool tmpBool2;
        if (File.Exists(out_Renamed.FullName))
        {
            File.Delete(out_Renamed.FullName);
            tmpBool2 = true;
        }
        else if (Directory.Exists(out_Renamed.FullName))
        {
            Directory.Delete(out_Renamed.FullName);
            tmpBool2 = true;
        }
        else
        {
        ...
        if (tmpBool && !tmpBool2) throw new IOException("Could not reset file");
        ...
        if (bitDepth <= 0 || bitDepth > 31) throw new IOException("PGX supports only bit-depth between " + "1 and 31");
        if (bitDepth <= 8)
            packBytes = 1;
        else if (bitDepth <= 16)
            packBytes = 2;
        ...
        // Writes PGX header
        var tmpString =
            "PG " + "ML " + (this.isSigned ? "- " : "+ ") + bitDepth + " " + w + " " + h + "\n"; // component height

        var tmpByte = Encoding.ASCII.GetBytes(tmpString);
        for (var i = 0; i < tmpByte.Length; i++) this.out_Renamed.WriteByte(tmpByte[i]);
    ...
    /// <seealso cref="DataBlk">
    /// </seealso>
    public ImgWriterPGX(string fname, BlkImgDataSrc imgSrc, int c, bool isSigned) : this(new FileInfo(fname), imgSrc, c,
        isSigned)
    {
    }
    ...
    public override void close()
    {
        ...
        {
            // Goto end of file
            out_Renamed.Seek(out_Renamed.Length, SeekOrigin.Begin);
            // Fill with 0s
            for (i = offset + w * h * packBytes - (int)out_Renamed.Length; i > 0; i--) out_Renamed.WriteByte(0);
        }
    ...
    public override void write(int ulx, int uly, int w, int h)
    {
        ...
        // Check line buffer
        if (buf == null || buf.Length < packBytes * w) buf = new byte[packBytes * w]; // Expand buffer
        ...
        switch (packBytes)
        {
            case 1: // Samples packed into 1 byte
                ...
                for (i = 0; i < h; i++)
                {
                    ...
                    out_Renamed.Seek(offset + this.w * (uly + tOffy + i) + ulx + tOffx, SeekOrigin.Begin);
            ...
            case 2: // Samples packed in to 2 bytes (short)
                ...
                {
                    // Skip to beggining of line in file
                    out_Renamed.Seek(offset + 2 * (this.w * (uly + tOffy + i) + ulx + tOffx), SeekOrigin.Begin);
                    // Write all bytes in the line
                    if (fracbits == 0)
            ...
            case 4:
                ...
                {
                    // Skip to beggining of line in file
                    out_Renamed.Seek(offset + 4 * (this.w * (uly + tOffy + i) + ulx + tOffx), SeekOrigin.Begin);
                    // Write all bytes in the line
                    if (fracbits == 0)
    ...
    public override string ToString()
    {
        ...
        return "ImgWriterPGX: WxH = " + w + "x" + h + ", Component = " + c + ", Bit-depth = " + bitDepth +
