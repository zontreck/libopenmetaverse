...
public class ImgWriterPGM : ImgWriter
{
    ...
    /// </param>
    public ImgWriterPGM(FileInfo out_Renamed, BlkImgDataSrc imgSrc, int c)
    {
        // Check that imgSrc is of the correct type
        // Check that the component index is valid
        if (c < 0 || c >= imgSrc.NumComps) throw new ArgumentException("Invalid number of components");
    ...
    ///     The index of the component from where to get the data.
    /// </param>
    public ImgWriterPGM(string fname, BlkImgDataSrc imgSrc, int c) : this(new FileInfo(fname), imgSrc, c)
    {
    }
    ...
    public override void close()
    {
        ...
        if (out_Renamed.Length != w * h + offset)
        {
            ...
            // Fill with 0s
            for (i = offset + w * h - (int)out_Renamed.Length; i > 0; i--) out_Renamed.WriteByte(0);
        }
    ...
    public override void write(int ulx, int uly, int w, int h)
    {
        ...
        if (downShift < 0) downShift = 0;

        // Check line buffer
        if (buf == null || buf.Length < w) buf = new byte[w]; // Expand buffer
