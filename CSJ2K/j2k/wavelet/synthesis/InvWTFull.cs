...
public class InvWTFull : InverseWT
{
    ...
    private readonly ProgressWatch pw;
    ...
    ///     needed basis.
    /// </summary>
    private readonly Dictionary<int, bool[]> reversible = new();
    ...
    private bool isSubbandReversible(Subband subband)
    {
        if (subband.isNode)
            ...
            return isSubbandReversible(subband.LL) && isSubbandReversible(subband.HL) &&
                   isSubbandReversible(subband.LH) && isSubbandReversible(subband.HH) &&
                   ((SubbandSyn)subband).hFilter.Reversible && ((SubbandSyn)subband).vFilter.Reversible;
    ...
    ///         If this number is
    ///         <i>
    ///             b</b> then for unsigned data the nominal range
    ...
    public override DataBlk getInternCompData(DataBlk blk, int c)
    {
        ...
        if (reconstructedComps[c] == null)
        {
            ...
            switch (dtype)
            {
                case DataBlk.TYPE_FLOAT:
                    reconstructedComps[c] =
                        new DataBlkFloat(0, 0, getTileCompWidth(tIdx, c), getTileCompHeight(tIdx, c));
                    break;
            ...
            if (pw != null && c == src.NumComps - 1) pw.terminateProgressWatch();
        }
    ...
    public override DataBlk getCompData(DataBlk blk, int c)
    {
        ...
        switch (blk.DataType)
        {
            case DataBlk.TYPE_INT:
                ...
                if (dst_data_int == null || dst_data_int.Length < blk.w * blk.h) dst_data_int = new int[blk.w * blk.h];
    ...
    private void waveletTreeReconstruction(DataBlk img, SubbandSyn sb, int c)
    {
        ...
        if (!sb.isNode)
        {
            ...
            if (sb.w == 0 || sb.h == 0) return; // If empty subband do nothing
            ...
            for (n = 0; n < ncblks.x; n++)
            {
                ...
                for (i = subbData.h - 1; i >= 0; i--)
                    // CONVERSION PROBLEM
                    Array.Copy((Array)src_data, subbData.offset + i * subbData.scanw, (Array)dst_data,
                        (subbData.uly + i) * img.w + subbData.ulx, subbData.w);
            }
        }
        else if (sb.isNode)
        {
    ...
    public override void setTile(int x, int y)
    {
        ...
        for (var c = 0; c < nc; c++) ndl[c] = src.getSynSubbandTree(tIdx, c).resLvl;
        ...
        if (pw != null) pw.initProgressWatch(0, cblkToDecode, "Decoding tile " + tIdx + "...");
    }
