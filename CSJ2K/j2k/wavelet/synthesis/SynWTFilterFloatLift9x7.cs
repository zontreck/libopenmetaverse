...
public class SynWTFilterFloatLift9x7 : SynWTFilterFloat
{
    ...
    /// </seealso>
    public override void synthetize_lpf(float[] lowSig, int lowOff, int lowLen, int lowStep, float[] highSig,
        int highOff, int highLen, int highStep, float[] outSig, int outOff, int outStep)
    {
        ...
        if (outLen % 2 == 0) outSig[ik] = highSig[hk] / KH - 2 * GAMMA * outSig[ik - outStep];
        ...
        if (outLen > 1) outSig[ik] -= 2 * BETA * outSig[ik + outStep];
        ik += iStep;
        ...
        //Handle head boundary effect if input signal has odd length
        if (outLen % 2 == 1 && outLen > 2) outSig[ik] -= 2 * BETA * outSig[ik - outStep];
        ...
        if (outLen % 2 == 0) outSig[ik] -= 2 * ALPHA * outSig[ik - outStep];
    }
    ...
    /// </seealso>
    public override void synthetize_hpf(float[] lowSig, int lowOff, int lowLen, int lowStep, float[] highSig,
        int highOff, int highLen, int highStep, float[] outSig, int outOff, int outStep)
    {
        int i;
        var outLen = lowLen + highLen; //Length of the output signal
