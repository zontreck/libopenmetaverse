...
public abstract class AnWTFilter : WaveletFilter
{
    ...
    private static readonly string[][] pinfo =
    {
        new[]
        {
            ...
            " If this option is used, a component transformation is applied " + "to the three first components.\n" +
            "<tile-component idx>: see general note\n" +
            "<id>: ',' separates horizontal and vertical filters, ':' separates" +
            " decomposition levels filters. JPEG 2000 part 1 only supports w5x3" + " and w9x7 filters.",
            null
        }
    };
    ...
    private static float[] upsampleAndConvolve(float[] in_Renamed, float[] wf, float[] out_Renamed)
    {
        ...
        if (out_Renamed == null) out_Renamed = new float[in_Renamed.Length * 2 + wf.Length - 2];
        ...
        for (i = 0, maxi = in_Renamed.Length * 2 + wf.Length - 2; i < maxi; i++)
        {
            ...
            for (j = 2 * k - i + wf.Length - 1; k < maxk; k++, j += 2) tmp += in_Renamed[k] * wf[j];
            // Store result
            out_Renamed[i] = tmp;
        }
