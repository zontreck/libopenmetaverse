...
public class SubbandAn : Subband
{
        ...
        if (l2Norm < 0)
        {
            ...
            if (isNode)
            {
                ...
                else if (subb_HH.l2Norm < 0f)
                {
                    ...
                    if (subb_HH.l2Norm >= 0f) l2Norm = 0f; // We are on a node, any non-neg value OK
                }
                else
                {
                    // There is an error! If all childs have non-negative
	...
	private void calcL2Norms()
    {
        ...
        while (l2Norm < 0f)
        {
            ...
            // and column L2-norms
            acc = 0.0;
            for (i = wfs[0].Length - 1; i >= 0; i--) acc += wfs[0][i] * wfs[0][i];
            //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
            l2n = (float)Math.Sqrt(acc);
            // Compute column L2-norm
            acc = 0.0;
            for (i = wfs[1].Length - 1; i >= 0; i--) acc += wfs[1][i] * wfs[1][i];
            //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
            l2n *= (float)Math.Sqrt(acc);
            // Release waveforms
            wfs[0] = null;
