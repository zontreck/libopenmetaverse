...
public class AnWTFilterIntLift5x3 : AnWTFilterInt
{
    ...
    /// </param>
    public override void analyze_lpf(int[] inSig, int inOff, int inLen, int inStep, int[] lowSig, int lowOff,
        int lowStep, int[] highSig, int highOff, int highStep)
    {
        ...
        if (inLen % 2 == 0) highSig[hk] = inSig[ik] - ((2 * inSig[ik - inStep]) >> 1);
    ...
    /// </seealso>
    public override void analyze_hpf(int[] inSig, int inOff, int inLen, int inStep, int[] lowSig, int lowOff,
        int lowStep, int[] highSig, int highOff, int highStep)
    {
        int i;
        var iStep = 2 * inStep; //Subsampling in inSig
        ...
        if (inLen % 2 == 1 && inLen > 1) highSig[hk] = inSig[ik] - inSig[ik - inStep];
