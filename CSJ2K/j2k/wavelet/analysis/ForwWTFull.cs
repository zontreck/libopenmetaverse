...
public class ForwWTFull : ForwardWT
{
    ...
    public ForwWTFull(BlkImgDataSrc src, EncoderSpecs encSpec, int cb0x, int cb0y) : base(src)
    {
        ...
        for (var i = 0; i < ntiles; i++) subbTrees[i] = new SubbandAn[ncomp];
        lastn = new int[ncomp];
        lastm = new int[ncomp];
    }
    ...
    public override CBlkWTData getNextCodeBlock(int c, CBlkWTData cblk)
    {
        ...
        if (cblk != null) dst_data = cblk.Data;

        // Get the next code-block
        cblk = getNextInternCodeBlock(c, cblk);

        if (cblk == null) return null; // No more code-blocks in current tile for component
        // c
        ...
        if (intData)
        {
            ...
            if (dst_data_int == null || dst_data_int.Length < cblk.w * cblk.h) dst_data = new int[cblk.w * cblk.h];
        }
        else
        {
            // float data
    ...
    private SubbandAn getNextSubband(int c)
    {
        ...
        if (nextsb == null)
        {
            ...
            if (!nextsb.isNode) return nextsb;
        }
        ...
        if (intData)
        {
            ...
            if (subband.ulcy % 2 == 0)
                ...
                for (j = 0; j < w; j++)
                {
                    ...
                    subband.vFilter.analyze_lpf(tmpVector, 0, h, 1, data, offset, band_w, data,
                        offset + (h + 1) / 2 * band_w, band_w);
                }
            else
                ...
                for (j = 0; j < w; j++)
                {
                    ...
                    for (i = 0; i < h; i++)
                        tmpVector[i] = data[offset + i * band_w];
                    subband.vFilter.analyze_hpf(tmpVector, 0, h, 1, data, offset, band_w, data, offset + h / 2 * band_w,
                        band_w);
                }
    ...
    public override SubbandAn getAnSubbandTree(int t, int c)
    {
        ...
        {
            subbTrees[t][c] = new SubbandAn(getTileCompWidth(t, c), getTileCompHeight(t, c), getCompULX(c),
                getCompULY(c), getDecompLevels(t, c), getHorAnWaveletFilters(t, c), getVertAnWaveletFilters(t, c));
