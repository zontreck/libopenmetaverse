...
public abstract class ForwardWT : ImgDataAdapter, ForwWT, CBlkWTDataSrc
{
    ...
    public static ForwardWT createInstance(BlkImgDataSrc src, ParameterList pl, EncoderSpecs encSpec)
    {
        ...
        if (pl.getParameter("Wcboff") == null)
            throw new ApplicationException("You must specify an argument to the '-Wcboff' " +
                                           "option. See usage with the '-u' option");
        var stk = new SupportClass.Tokenizer(pl.getParameter("Wcboff"));
        if (stk.Count != 2)
            throw new ArgumentException("'-Wcboff' option needs two" + " arguments. See usage with " +
                                        "the '-u' option.");
        var cb0x = 0;
        str = stk.NextToken();
        try
        ...
        if (cb0x < 0 || cb0x > 1) throw new ArgumentException("Invalid horizontal " + "code-block partition origin.");
        var cb0y = 0;
        str = stk.NextToken();
        try
        {
        ...
        if (cb0y < 0 || cb0y > 1) throw new ArgumentException("Invalid vertical " + "code-block partition origin.");
        if (cb0x != 0 || cb0y != 0)
