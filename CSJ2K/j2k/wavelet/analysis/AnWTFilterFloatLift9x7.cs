...
public class AnWTFilterFloatLift9x7 : AnWTFilterFloat
{
    ...
    private static readonly float[] LPSynthesisFilter =
        { -0.091272f, -0.057544f, 0.591272f, 1.115087f, 0.591272f, -0.057544f, -0.091272f };
    ...
    private static readonly float[] HPSynthesisFilter =
    ...
    /// </param>
    public override void analyze_lpf(float[] inSig, int inOff, int inLen, int inStep, float[] lowSig, int lowOff,
        int lowStep, float[] highSig, int highOff, int highStep)
    {
        ...
        if (inLen % 2 == 0) highSig[hk] = inSig[ik] + 2 * ALPHA * inSig[ik - inStep];
        ...
        if (inLen % 2 == 1 && inLen > 2) lowSig[lk] = inSig[ik] + 2 * BETA * highSig[hk - highStep];
        ...
        //Handle head boundary effect if input signal has even length
        if (inLen % 2 == 0) highSig[hk] += 2 * GAMMA * lowSig[lk];
        ...
        if (inLen > 1) lowSig[lk] += 2 * DELTA * highSig[hk];
        ...
        //Handle head boundary effect if input signal has odd length
        if (inLen % 2 == 1 && inLen > 2) lowSig[lk] += 2 * DELTA * highSig[hk - highStep];
        ...
        if (inLen % 2 == 1 && inLen != 1) lowSig[lk] *= KL;
    }
    ...
    /// </seealso>
    public override void analyze_hpf(float[] inSig, int inOff, int inLen, int inStep, float[] lowSig, int lowOff,
        int lowStep, float[] highSig, int highOff, int highStep)
    {
        int i; // maxi removed
        var iStep = 2 * inStep; //Subsampling in inSig
        ...
        if (inLen % 2 == 1 && inLen > 1) highSig[hk] = inSig[ik] + 2 * ALPHA * inSig[ik - inStep];
        ...
        if (inLen > 1 && inLen % 2 == 0) lowSig[lk] += DELTA * 2 * highSig[hk];
        ...
        //coefficient (if input signal is length one filter is identity)
        if (inLen % 2 == 1 && inLen != 1) highSig[hk] *= KH;
    }
