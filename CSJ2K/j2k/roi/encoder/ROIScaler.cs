...
public class ROIScaler : ImgDataAdapter, CBlkQuantDataSrcEnc
{
    ...
    private static readonly string[][] pinfo =
    {
        new[]
        {
            ...
            " or [<component idx>] C <centre column> <centre row> " + "<radius> or [<component idx>] A <filename>",
            ...
            "values correspond to ROI coefficients. The PGM file must have " + "the size as the image. " +
        ...
        new[]
        {
            ...
            "belongs to the ROI, 1 means the two lowest etc. (-1 deactivates" + " the option)",
            "-1"
        },
        new[]
        {
    ...
    {
        var roiVector = ArrayList.Synchronized(new ArrayList(10));
    ...
    /// </param>
    /// <param name="roiVector">
    ///     The vcector containing the ROI parsed from the cmd line
    /// </param>
    /// <returns>
    ///     The ROIs specified in roiopt
    /// </returns>
    protected internal static ArrayList parseROIs(string roiopt, int nc, ArrayList roiVector)
    {
        //ROI[] ROIs;
        ROI roi;
        ...
        while (stok.HasMoreTokens())
        {
            ...
            switch (word[0])
            {
                ...
                case 'A': // ROI wth arbitrary shape
                    ...
                    {
                        throw new ApplicationException("Cannot read PGM file with ROI");
                    }
                ...
                default:
                    throw new ApplicationException("Bad parameters for ROI nr " + roiVector.Count);
            }
        }
    ...
    private void calcMaxMagBits(EncoderSpecs encSpec)
    {
        ...
        for (var i = 0; i < nt; i++) maxMagBits[i] = new int[nc];
