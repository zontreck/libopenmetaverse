...
public class ArbROIMaskGenerator : ROIMaskGenerator
{
    ...
    public override bool getROIMask(DataBlkInt db, Subband sb, int magbits, int c)
    {
        ...
        for (j = h; j > 0; j--)
        {
            for (k = w; k > 0; k--, i--, bi--) maskData[bi] = mask[i];
            i -= wrap;
        }
        ...
        for (r = rois.Length - 1; r >= 0; r--)
            if (rois[r].comp == c)
            {
                ...
                else if (rois[r].rect)
                {
                    ...
                    for (k = h; k > 0; k--)
                    {
                        for (j = maxj; j > 0; j--, i--) mask[i] = curScalVal;
                        i -= wrap;
                    }
                }
                else
    ...
    private void decomp(Subband sb, int tilew, int tileh, int c)
    {
        ...
        for (j = h - 1; j >= 0; j--)
        {
            ...
            mi = lineoffs;
            for (k = w, pin = w - 1 + maxnSup; k > 0; k--, mi--, pin--) padLine[pin] = mask[mi];
            ...
            mi = lineoffs;
            for (k = hmax; k >= 0; k--, mi--) mask[mi] = high[k];
            for (k = lmax; k >= 0; k--, mi--) mask[mi] = low[k];
        }
        ...
        for (j = w - 1; j >= 0; j--)
        {
            ...
            // Get the line to transform from the mask
            mi = lineoffs;
            for (k = h, pin = k - 1 + maxnSup; k > 0; k--, mi -= tilew, pin--) padLine[pin] = mask[mi];
            lastpin = maxnSup + highFirst + 2 * lmax + lpSup;
            for (k = lmax; k >= 0; k--, lastpin -= 2)
            {
                // Low frequency samples
            ...
            // Put the lows and highs back
            mi = lineoffs;
            for (k = hmax; k >= 0; k--, mi -= tilew) mask[mi] = high[k];
            for (k = lmax; k >= 0; k--, mi -= tilew) mask[mi] = low[k];
        }
