...
public class ModuleSpec : ICloneable
{
    ...
    /// </summary>
    protected internal Hashtable tileCompVal;

    /// <summary>
    ///     The default value for each tile. Null if no tile specific value is
    ///     defined
    /// </summary>
    protected internal object[] tileDef;
    ...
    public ModuleSpec(int nt, int nc, byte type)
    {
        ...
        for (var i = 0; i < nt; i++) specValType[i] = new byte[nc];
        switch (type)
        {
            case SPEC_TYPE_TILE:
    ...
    public virtual object Clone()
    {
        ...
        {
            throw new ApplicationException("Error when cloning ModuleSpec instance");
        }

        // Create a copy of the specValType array
        ms.specValType = new byte[nTiles][];
        for (var i = 0; i < nTiles; i++) ms.specValType[i] = new byte[nComp];
        ...
        if (tileDef != null)
        {
            ...
            for (var t = 0; t < nTiles; t++) ms.tileDef[t] = tileDef[t];
        }

        // Create a copy of tileCompVal
        if (tileCompVal != null)
        {
            ms.tileCompVal = Hashtable.Synchronized(new Hashtable());
            string tmpKey;
            object tmpVal;
            //UPGRADE_TODO: Method 'java.util.Enumeration.hasMoreElements' was converted to 'System.Collections.IEnumerator.MoveNext' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javautilEnumerationhasMoreElements'"
            for (var e = tileCompVal.Keys.GetEnumerator(); e.MoveNext();)
            {
    ...
    public virtual void rotate90(Coord anT)
    {
        ...
        {
            var tmptcv = Hashtable.Synchronized(new Hashtable());
            string tmpKey;
            object tmpVal;
            int btIdx, atIdx;
            int i1, i2;
    ...
    public virtual void setCompDef(int c, object value_Renamed)
    {
        if (specType == SPEC_TYPE_TILE)
        {
            ...
            var errMsg = "Option whose value is '" + value_Renamed + "' cannot be " +
                         "specified for components as it is a 'tile only' specific " + "option";
            throw new ApplicationException(errMsg);
        }

        if (compDef == null) compDef = new object[nComp];
    ...
    {
        if (specType == SPEC_TYPE_TILE) throw new ApplicationException("Illegal use of ModuleSpec class");
    ...
    public virtual void setTileDef(int t, object value_Renamed)
    {
        ...
        if (tileDef == null) tileDef = new object[nTiles];
    ...
    {
        if (specType == SPEC_TYPE_COMP) throw new ApplicationException("Illegal use of ModuleSpec class");
    ...
    public virtual void setTileCompVal(int t, int c, object value_Renamed)
    {
        ...
        if (tileCompVal == null)
            tileCompVal = Hashtable.Synchronized(new Hashtable());
    ...
    {
        if (specType != SPEC_TYPE_TILE_COMP) throw new ApplicationException("Illegal use of ModuleSpec class");
        return getSpec(t, c);
    }
