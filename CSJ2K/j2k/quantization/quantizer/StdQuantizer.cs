...
public class StdQuantizer : Quantizer
{
    ...
    public override CBlkWTData getNextInternCodeBlock(int c, CBlkWTData cblk)
    {
        ...
        if (cblk == null) cblk = new CBlkWTDataInt();
        ...
        if (intq)
        {
            ...
            if (cblk == null) return null; // No more code-blocks in current tile for comp.
    ...
    protected internal override void calcSbParams(SubbandAn sb, int c)
    {
        ...
        if (!sb.isNode)
        {
            ...
            else
            {
                ...
                if (isDerived(tIdx, c))
                    //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                    sb.stepWMSE = baseStep * baseStep * (float)Math.Pow(2, (sb.anGainExp - sb.level) << 1) * sb.l2Norm *
                                  sb.l2Norm;
                else
    ...
    private static float convertFromExpMantissa(int ems)
    {
        ...
        // able to represent 2**31)
        //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
        return (-1f - (ems & QSTEP_MAX_MANTISSA) / (float)(1 << QSTEP_MANTISSA_BITS)) /
    ...
    public override int getMaxMagBits(int c)
    {
        ...
        if (isReversible(tIdx, c)) return getMaxMagBitsRev(sb, c);
