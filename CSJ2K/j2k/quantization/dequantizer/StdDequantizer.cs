...
public class StdDequantizer : Dequantizer
{
    ...
    {
        if (utrb.Length != src.NumComps) throw new ArgumentException("Invalid rb argument");
        qsss = decSpec.qsss;
        qts = decSpec.qts;
        gbs = decSpec.gbs;
    }
    ...
    public override DataBlk getInternCodeBlock(int c, int m, int n, SubbandSyn sb, DataBlk cblk)
    {
        ...
        switch (outdtype)
        {
            ...
            case DataBlk.TYPE_FLOAT:
                ...
                inarr = inblk.DataInt;
                if (cblk == null) cblk = new DataBlkFloat();
        ...
        else
        {
            ...
            switch (outdtype)
            {
                ...
                case DataBlk.TYPE_FLOAT:
                    ...
                    w = cblk.w;
                    h = cblk.h;
                    for (j = w * h - 1, k = inblk.offset + (h - 1) * inblk.scanw + w - 1, jmin = w * (h - 1);
                         j >= 0;
                         jmin -= w)
                    {
                        for (; j >= jmin; k--, j--)
