        ...
        while (stk.HasMoreTokens())
        {
            ...
            switch (word[0])
            {
                ...
                case 'e': // expounded quantization step size specification
                    if (!word.ToUpper().Equals("reversible".ToUpper()) && !word.ToUpper().Equals("derived".ToUpper()) &&
                        !word.ToUpper().Equals("expounded".ToUpper()))
                        throw new ArgumentException("Unknown parameter " + "for " + "'-Qtype' option: " + word);

                    if (pl.getBooleanParameter("lossless") && (word.ToUpper().Equals("derived".ToUpper()) ||
                                                               word.ToUpper().Equals("expounded".ToUpper())))
                        throw new ArgumentException("Cannot use non " + "reversible " + "quantization with " +
                                                    "'-lossless' option");
