...
public class ByteInputBuffer
{
    ...
    public virtual void setByteArray(byte[] buf, int offset, int length)
    {
        ...
        if (buf == null)
        {
            if (length < 0 || count + length > this.buf.Length) throw new ArgumentException();
            if (offset < 0)
            {
                pos = count;
                count += length;
        ...
        {
            // New input buffer
            if (offset < 0 || length < 0 || offset + length > buf.Length) throw new ArgumentException();
            this.buf = buf;
            count = offset + length;
            pos = offset;
        }
    ...
    public virtual void addByteArray(byte[] data, int off, int len)
    {
        ...
        {
            // Check integrity
            if (len < 0 || off < 0 || len + off > buf.Length) throw new ArgumentException();
            // Copy new data
