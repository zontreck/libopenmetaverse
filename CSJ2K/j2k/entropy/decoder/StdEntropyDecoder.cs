...
public class StdEntropyDecoder : EntropyDecoder
{
	...
	public override DataBlk getCodeBlock(int c, int m, int n, SubbandSyn sb, DataBlk cblk)
    {
        ...
        if (mQuit != -1 && mQuit * 3 - 2 < npasses) npasses = mQuit * 3 - 2;
        ...
        if (curbp >= 0 && npasses > 0)
        {
            isterm = (options & StdEntropyCoderOptions.OPT_TERM_PASS) != 0 ||
                     ((options & StdEntropyCoderOptions.OPT_BYPASS) != 0 &&
                      31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - srcblk.skipMSBP >= curbp);
        ...
        if (!error || !doer)
            ...
            {
                if ((options & StdEntropyCoderOptions.OPT_BYPASS) != 0 &&
                    curbp < 31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - srcblk.skipMSBP)
                {
        ...
        if (error && doer)
        {
            if (verber)
                FacilityManager.getMsgLogger().printmsg(MsgLogger_Fields.WARNING,
                    "Error detected at bit-plane " + curbp + " in code-block (" + m + "," + n + "), sb_idx " +
                    sb.sbandIdx + ", res. level " + sb.resLvl + ". Concealing...");
            conceal(cblk, curbp);
        }
	...
	private bool sigProgPass(DataBlk cblk, MQDecoder mq, int bp, int[] state, int[] zc_lut, bool isterm)
    {
        ...
        if (isterm && (options & StdEntropyCoderOptions.OPT_PRED_TERM) != 0) error = mq.checkPredTerm();
	...
	private bool rawSigProgPass(DataBlk cblk, ByteToBitInput bin, int bp, int[] state, bool isterm)
    {
        ...
        if (isterm && (options & StdEntropyCoderOptions.OPT_PRED_TERM) != 0) error = bin.checkBytePadding();
