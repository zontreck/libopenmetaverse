...
public class EBCOTRateAllocator : PostCompRateAllocator
{
    ...
    public EBCOTRateAllocator(CodedCBlkDataSrcEnc src, LayersInfo lyrs, CodestreamWriter writer, EncoderSpecs encSpec,
        ParameterList pl) : base(src, lyrs.TotNumLayers, writer, encSpec)
    {
        int minsbi, maxsbi;
        int i;
        SubbandAn sb, sb2;
        ...
        for (var i2 = 0; i2 < nt; i2++) cblks[i2] = new CBlkRateDistStats[nc][][][];
        truncIdxs = new int[nt][][][][][];
        for (var i3 = 0; i3 < nt; i3++)
        {
            truncIdxs[i3] = new int[num_Layers][][][][];
            for (var i4 = 0; i4 < num_Layers; i4++) truncIdxs[i3][i4] = new int[nc][][][];
        }
        ...
        for (var t = 0; t < nt; t++)
        {
            ...
            for (var c = 0; c < nc; c++)
            {
                ...
                if (numPrec == null)
                {
                    ...
                    for (var i5 = 0; i5 < nt; i5++) tmpArray[i5] = new Coord[nc][];
                    numPrec = tmpArray;
                }

                if (numPrec[t][c] == null) numPrec[t][c] = new Coord[mrl];
                ...
                for (l = 0; l < num_Layers; l++) truncIdxs[t][l][c] = new int[mrl][][];
                ...
                for (var r = 0; r < mrl; r++)
                {
                    ...
                    if (trx1 > trx0)
                        //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                        numPrec[t][c][r].x = (int)Math.Ceiling((trx1 - cb0x) / twoppx) -
                                             (int)Math.Floor((trx0 - cb0x) / twoppx);
                    else
                        numPrec[t][c][r].x = 0;
                    if (try1 > try0)
                        //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                        numPrec[t][c][r].y = (int)Math.Ceiling((try1 - cb0y) / twoppy) -
                    ...
                    for (l = 0; l < num_Layers; l++) truncIdxs[t][l][c][r] = new int[maxsbi][];
                    ...
                    for (s = minsbi; s < maxsbi; s++)
                    {
                        ...
                        for (l = 0; l < num_Layers; l++)
                        {
                            ...
                            for (i = 0; i < cblkPerSubband; i++) truncIdxs[t][l][c][r][s][i] = -1;
                        }
    ...
    public override void initialize()
    {
        ...
        for (var t = 0; t < numTiles; t++)
        {
            ...
            if (((string)encSpec.sops.getTileDef(t)).ToUpper().Equals("on".ToUpper())) avgPktLen += Markers.SOP_LENGTH;
            // Add EPH length if set
            if (((string)encSpec.ephs.getTileDef(t)).ToUpper().Equals("on".ToUpper())) avgPktLen += Markers.EPH_LENGTH;
        ...
        for (n = num_Layers - 1; n >= 0; n--) layers[n] = new EBCOTLayer();
        ...
        while (newbytes < minlsz)
        {
            ...
            {
                if (newbytes <= 0)
                    throw new ArgumentException("Overall target bitrate too " + "low, given the current " +
        ...
        if (nValidProg == 0)
            throw new ApplicationException("Unable to initialize rate allocator: No " +
            ...
            if (encSpec.pocs.isTileSpecified(t))
            {
                ...
                for (var prg = 0; prg < prog1.Length; prg++)
                    if (prog1[prg].lye > num_Layers)
                        prog1[prg].lye = num_Layers;
                if (nValidProg == 0)
                    throw new ApplicationException("Unable to initialize rate allocator:" +
                                                   " No default progression type has been " + "defined for tile " + t);
            } // End loop on tiles
    ...
    private void getAllCodeBlocks()
    {
        ...
        var cblkToEncode = 0;
        var nEncCblk = 0;
        var pw = FacilityManager.ProgressWatch;
        ...
        for (t = 0; t < numTiles; t++)
        {
            ...
            if (pw != null) pw.initProgressWatch(0, cblkToEncode, "Encoding tile " + t + "...");
            ...
            for (c = 0; c < numComps; c++)
                ...
                while ((ccb = src.getNextCodeBlock(c, ccb)) != null)
                {
                    ...
                    for (k = ccb.nVldTrunc - 1; k >= 0; k--)
                    {
                        ...
                        for (; sidx > last_sidx; sidx--) RDSlopesRates[sidx] += ccb.truncRates[ccb.truncIdxs[k]];
            ...
            if (pw != null) pw.terminateProgressWatch();
        ...
        for (var l = 0; l < num_Layers; l++)
        {
            ...
            else
            {
                if (l <= 0 || l >= num_Layers - 1)
                    throw new ArgumentException("The first and the" + " last layer " + "thresholds" +
                                                " must be optimized");
            ...
            for (var t = 0; t < nt; t++)
            {
                ...
                for (var c = 0; c < nc; c++)
                {
                    ...
                    while (sb.subb_LL != null) sb = sb.subb_LL;
                    ...
                    for (var r = 0; r < mrl; r++)
                    {
                        ...
                        for (var p = 0; p < nPrec; p++)
                        {
                            ...
                            hBuff = pktEnc.encodePacket(l + 1, c, r, t, cblks[t][c][r], truncIdxs[t][l][c][r], hBuff,
                                bBuff, p);
                            if (pktEnc.PacketWritable)
                            {
                                tmp = bsWriter.writePacketHead(hBuff.Buffer, hBuff.Length, true, sopUsed, ephUsed);
                                tmp += bsWriter.writePacketBody(pktEnc.LastBodyBuf, pktEnc.LastBodyLen, true,
                                    pktEnc.ROIinPkt, pktEnc.ROILen);
                                actualBytes += tmp;
                                tileLengths[t] += tmp;
                            }
        ...
        for (var t = 0; t < nt; t++)
        {
            ...
            for (var prg = 0; prg < prog.Length; prg++)
            {
                ...
                switch (prog[prg].type)
                {
                    ...
                    default:
                        throw new ApplicationException("Unsupported bit stream progression type");
        ...
        for (var r = rs; r < re; r++)
        {
            ...
            for (var c = cs; c < ce; c++)
            {
                ...
                for (var p = 0; p < nPrec; p++)
                {
                    ...
                    sb = src.getAnSubbandTree(t, c);
                    for (var i = mrl[c]; i > r; i--) sb = sb.subb_LL;
                    ...
                    if (pktEnc.PacketWritable)
                    {
                        ...
                        bsWriter.writePacketBody(pktEnc.LastBodyBuf, pktEnc.LastBodyLen, false, pktEnc.ROIinPkt,
                            pktEnc.ROILen);
                    }
                } // End loop on precincts
            } // End loop on components
    ...
    public virtual void writeLyResCompPos(int t, int rs, int re, int cs, int ce, int[][] lys, int lye)
    {
        ...
        for (var c = cs; c < ce; c++)
        {
            ...
            for (var p = 0; p < nPrec; p++)
            {
                ...
                sb = src.getAnSubbandTree(t, c);
                for (var i = mrl; i > r; i--) sb = sb.subb_LL;
    ...
    public virtual void writePosCompResLy(int t, int rs, int re, int cs, int ce, int[][] lys, int lye)
    {
        ...
        for (var c = cs; c < ce; c++)
        {
            ...
            for (var r = rs; r < re; r++)
            {
                if (r > mrl)
                    continue;
                if (r < lys[c].Length && lys[c][r] < minlys) minlys = lys[c][r];
            ...
            for (var px = 0; px <= pxend; px++)
            {
                ...
                for (var c = cs; c < ce; c++)
                {
                    ...
                    for (var r = rs; r < re; r++)
                    {
                        ...
                        if (r > mrl)
                            continue;
                        if (nextPrec[c][r] >= numPrec[t][c][r].x * numPrec[t][c][r].y) continue;
        ...
        for (var c = cs; c < ce; c++)
        {
            ...
            for (var r = rs; r < re; r++)
            {
                ...
                if (nextPrec[c][r] < numPrec[t][c][r].x * numPrec[t][c][r].y - 1)
                    throw new ApplicationException("JJ2000 bug: One precinct at least has " +
                                                   "not been written for resolution level " + r + " of component " + c +
                                                   " in tile " + t + ".");
            }
        }
    }
            ...
            for (var c = 0; c < nc; c++)
            {
                ...
                for (var r = 0; r < numLvls; r++)
                {
                    ...
                    for (var p = 0; p < nPrec; p++)
                    {
                        ...
                        hBuff = pktEnc.encodePacket(layerIdx + 1, c, r, t, cblks[t][c][r], truncIdxs[t][layerIdx][c][r],
                            hBuff, bBuff, p);
                        ...
                        if (pktEnc.PacketWritable)
                        {
                            ...
                            actualBytes += bsWriter.writePacketBody(bBuff, pktEnc.LastBodyLen, true, pktEnc.ROIinPkt,
                                pktEnc.ROILen);
                        }
                    } // end loop on precincts
        ...
        int precinctIdx)
    {
        ...
        sb = subb;
        while (sb.subb_HH != null) sb = sb.subb_HH;
        minsbi = lvlIdx == 0 ? 0 : 1;
        maxsbi = lvlIdx == 0 ? 1 : 4;
