...
public class LayersInfo
{
    ...
    {
        if (brate <= 0) throw new ArgumentException("Overall target bitrate must " + "be a positive number");
        totbrate = brate;
    }
    ...
    public virtual void addOptPoint(float brate, int elyrs)
    {
        ...
        if (brate <= 0) throw new ArgumentException("Target bitrate must be positive");
        if (elyrs < 0) throw new ArgumentException("The number of extra layers " + "must be 0 or more");
        if (nopt > 0 && optbrate[nopt - 1] >= brate)
            throw new ArgumentException("New optimization point must have " + "a target bitrate higher than the " +
                                        "preceding one");
