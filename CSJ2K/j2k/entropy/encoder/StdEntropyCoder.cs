...
using ThreadPool = CSJ2K.j2k.util.ThreadPool;
...
public class StdEntropyCoder : EntropyCoder
{
    ...
    * */
    ...
    private static readonly int RLC_MASK_R1R2 = STATE_SIG_R1 | STATE_SIG_R2 | STATE_VISITED_R1 | STATE_VISITED_R2 |
    ...
    private class Compressor : IThreadRunnable
    {
        ...
        ///     The index of this compressor.
        /// </returns>
        public virtual int Idx => idx;

        public StdEntropyCoder Enclosing_Instance { get; private set; }
...
#endif
                compressCodeBlock(c, ccb, Enclosing_Instance.srcblkT[idx], Enclosing_Instance.mqT[idx],
                    Enclosing_Instance.boutT[idx], Enclosing_Instance.outT[idx], Enclosing_Instance.stateT[idx],
                    Enclosing_Instance.distbufT[idx], Enclosing_Instance.ratebufT[idx],
                    Enclosing_Instance.istermbufT[idx], Enclosing_Instance.symbufT[idx],
                    Enclosing_Instance.ctxtbufT[idx], options, rev, lcType, tType);
#if DO_TIMING
    ...
    public StdEntropyCoder(CBlkQuantDataSrcEnc src, CBlkSizeSpec cblks, PrecinctSizeSpec pss, StringSpec bms,
        StringSpec mqrs, StringSpec rts, StringSpec css, StringSpec sss, StringSpec lcs, StringSpec tts) : base(src)
    {
        this.cblks = cblks;
        this.pss = pss;
        this.bms = bms;
        ...
        */
        ...
        if (nt > 0)
        {
            ...
            tsl = nt;
            tPool = new ThreadPool(nt, (int)SupportClass.ThreadClass.Current().Priority + THREADS_PRIORITY_INC,
                "StdEntropyCoder");
            idleComps = new ArrayList();
            completedComps = new ArrayList[src.NumComps];
            nBusyComps = new int[src.NumComps];
            finishedTileComponent = new bool[src.NumComps];
            for (i = src.NumComps - 1; i >= 0; i--) completedComps[i] = new ArrayList();
            for (i = 0; i < nt; i++) idleComps.Add(new Compressor(this, i));
        }
        else
        {
            tsl = 1;
        ...
        stateT = new int[tsl][];
        for (var i2 = 0; i2 < tsl; i2++) stateT[i2] = new int[(maxCBlkWidth + 2) * ((maxCBlkHeight + 1) / 2 + 2)];
        symbufT = new int[tsl][];
        ...
        for (var i5 = 0; i5 < tsl; i5++) distbufT[i5] = new double[32 * StdEntropyCoderOptions.NUM_PASSES];
        ratebufT = new int[tsl][];
        for (var i6 = 0; i6 < tsl; i6++) ratebufT[i6] = new int[32 * StdEntropyCoderOptions.NUM_PASSES];
        istermbufT = new bool[tsl][];
        for (var i7 = 0; i7 < tsl; i7++) istermbufT[i7] = new bool[32 * StdEntropyCoderOptions.NUM_PASSES];
        ...
        for (var i8 = 0; i8 < src.NumComps; i8++) precinctPartition[i8] = new bool[src.getNumTiles()];
	...
	public override CBlkRateDistStats getNextCodeBlock(int c, CBlkRateDistStats ccb)
    {
        ...
        if (tPool == null)
        {
            ...
            // Compress code-block
            compressCodeBlock(c, ccb, srcblkT[0], mqT[0], boutT[0], outT[0], stateT[0], distbufT[0], ratebufT[0],
                istermbufT[0], symbufT[0], ctxtbufT[0], opts[tIdx][c], isReversible(tIdx, c), lenCalc[tIdx][c],
                tType[tIdx][c]);
        ...
        while (!finishedTileComponent[c] && !(idleComps.Count == 0))
        {
            ...
            {
                // Initialize thread local variables
                if ((opts[tIdx][c] & StdEntropyCoderOptions.OPT_BYPASS) != 0 && boutT[cIdx] == null)
        ...
        bool[] istermbuf, int[] symbuf, int[] ctxtbuf, int options, bool rev, int lcType, int tType)
    {
        ...
        if ((options & StdEntropyCoderOptions.OPT_PRED_TERM) != 0 && tType != MQCoder.TERM_PRED_ER)
            throw new ArgumentException("Embedded error-resilient info " + "in MQ termination option " +
        ...
        switch (srcblk.sb.orientation)
        {
            ...
            default:
                throw new ApplicationException("JJ2000 internal error");
        }
        ...
        fs = FS_LOSSY;
        fm = FM_LOSSY;
        msew = Math.Pow(2, ((curbp - lmb) << 1) - MSE_LKP_FRAC_BITS) * srcblk.sb.stepWMSE * srcblk.wmseScaling;
        totdist = 0f;
        npass = 0;
        ltpidx = -1;
        ...
        if (curbp >= lmb)
        {
            ...
            if (rev && curbp == lmb) fs = FM_LOSSLESS;
            // We terminate if regular termination, last bit-plane, or next
            // bit-plane is "raw".
            istermbuf[npass] = (options & StdEntropyCoderOptions.OPT_TERM_PASS) != 0 || curbp == lmb ||
                               ((options & StdEntropyCoderOptions.OPT_BYPASS) != 0 &&
                                31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - skipbp >= curbp);
        ...
        while (curbp >= lmb)
        {
            ...
            {
                // No bypass coding
                totdist += sigProgPass(srcblk, mq, istermbuf[npass], curbp, state, fs, zc_lut, symbuf, ctxtbuf, ratebuf,
                    npass, ltpidx, options) * msew;
            }
            else
            {
                ...
                totdist += rawSigProgPass(srcblk, bout, istermbuf[npass], curbp, state, fs, ratebuf, npass, ltpidx,
                    options) * msew;
            }
            ...
            {
                // No bypass coding
                totdist += magRefPass(srcblk, mq, istermbuf[npass], curbp, state, fm, symbuf, ctxtbuf, ratebuf, npass,
                    ltpidx, options) * msew;
            }
            else
            {
                // Bypass ("raw") coding
                bout.PredTerm = (options & StdEntropyCoderOptions.OPT_PRED_TERM) != 0;
                totdist += rawMagRefPass(srcblk, bout, istermbuf[npass], curbp, state, fm, ratebuf, npass, ltpidx,
                    options) * msew;
            }
        ...
        ccb.selectConvexHull(ratebuf, distbuf,
            (options & (StdEntropyCoderOptions.OPT_BYPASS | StdEntropyCoderOptions.OPT_TERM_PASS)) != 0
                ? istermbuf
                : null, npass, rev);
        ...
        int[] zc_lut, int[] symbuf, int[] ctxtbuf, int[] ratebuf, int pidx, int ltpidx, int options)
    {
        ...
        for (s = nstripes - 1; s >= 0; s--, sk += kstep, sj += jstep)
        {
            sheight = s != 0
                ...
                : srcblk.h - (nstripes - 1) * StdEntropyCoderOptions.STRIPE_HEIGHT;
            stopsk = sk + srcblk.w;
        ...
        if ((options & StdEntropyCoderOptions.OPT_RESET_MQ) != 0) mq.resetCtxts();
        ...
        if (ltpidx >= 0) ratebuf[pidx] += ratebuf[ltpidx];
        // Finish length calculation if needed
        if (doterm) mq.finishLengthCalculation(ratebuf, pidx);
	...
	public virtual void initTileComp(int nt, int nc)
    {
        ...
        lenCalc = new int[nt][];
        for (var i3 = 0; i3 < nt; i3++) lenCalc[i3] = new int[nc];
        tType = new int[nt][];
        for (var i4 = 0; i4 < nt; i4++) tType[i4] = new int[nc];
        ...
        for (var c = 0; c < nc; c++)
        {
            ...
            if (lCalcType.Equals("near_opt"))
                ...
                throw new ArgumentException("Unrecognized or " + "unsupported MQ " + "length calculation.");
            ...
            else if (termType.ToUpper().Equals("predict".ToUpper()))
            {
                ...
                if ((opts[t][c] & (StdEntropyCoderOptions.OPT_TERM_PASS | StdEntropyCoderOptions.OPT_BYPASS)) == 0)
                    FacilityManager.getMsgLogger().printmsg(MsgLogger_Fields.INFO,
                        ...
                        "the end of code-blocks. The error " + "protection offered by this option will" +
                        " be very weak. Specify the " + "'Cterminate' " + "and/or 'Cbypass' option for " +
                        "increased error resilience.");
            }
            else
            {
                throw new ArgumentException("Unrecognized or " + "unsupported " + "MQ coder " + "termination.");
            }
    ...
    {
        {
            ...
            for (i = 0; i < 64; i++) ZC_LUT_LH[STATE_H_L_R1 | STATE_H_R_R1 | i] = 10;
            ...
            for (i = 1; i < 1 << (MR_LUT_BITS - 1); i++) MR_LUT[i] = 17;
            // Previous MR on, significance irrelevant
            for (; i < 1 << MR_LUT_BITS; i++) MR_LUT[i] = 18;
            ...
            for (i = 0; i < 1 << MSE_LKP_BITS_M1; i++)
            {
                ...
                FS_LOSSLESS[i] = (int)Math.Floor(deltaMSE * (1 << MSE_LKP_FRAC_BITS) + 0.5);
                val -= 1.5;
                deltaMSE -= val * val;
                //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                FS_LOSSY[i] = (int)Math.Floor(deltaMSE * (1 << MSE_LKP_FRAC_BITS) + 0.5);
            }
            ...
            for (i = 0; i < 1 << MSE_LKP_BITS; i++)
            {
                ...
                //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                FM_LOSSLESS[i] = (int)Math.Floor(deltaMSE * (1 << MSE_LKP_FRAC_BITS) + 0.5);
                val -= i < 1 << MSE_LKP_BITS_M1 ? 0.5 : 1.5;
                deltaMSE -= val * val;
                //UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
                FM_LOSSY[i] = (int)Math.Floor(deltaMSE * (1 << MSE_LKP_FRAC_BITS) + 0.5);
            }
        }
    }
}