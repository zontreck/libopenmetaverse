...
public abstract class PostCompRateAllocator : ImgDataAdapter
{
    ...
    private static readonly string[][] pinfo =
    {
        ...
        new[]
        {
            ...
            "If the 'sl' (i.e. 'single layer') argument is specified, the " + "generated codestream will" +
            " only contain one layer (with a bit rate specified thanks to the" + " '-rate' or 'nbytes' options).",
            "0.015 +20 2.0 +10"
        }
    };
    ...
    private static LayersInfo parseAlayers(string params_Renamed, float rate)
    {
        ...
        stok = new SupportClass.StreamTokenizerSupport(new StringReader(params_Renamed));
        ...
        {
            throw new ApplicationException("An IOException has ocurred where it " + "should never occur");
        }
            ...
            switch (stok.ttype)
            {
                ...
                case '+':
                    if (!ratepending || islayer)
                        throw new ArgumentException("Layer parameter without " + "previous rate parameter " +
                                                    "in 'Alayers' option");
        ...
        if (islayer) throw new ArgumentException("Error parsing 'Alayers' " + "option");
        if (ratepending)
            try
            {
                lyrs.addOptPoint(r, 0);
