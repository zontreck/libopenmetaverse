...
public abstract class BufferedRandomAccessFile : RandomAccessIO, EndianType
{
    ...
    /// </exception>
    protected internal BufferedRandomAccessFile(FileInfo file, string mode, int bufferSize)
    {
        fileName = file.Name;
    ...
    ///     If an I/O error ocurred.
    /// </exception>
    protected internal BufferedRandomAccessFile(FileInfo file, string mode) : this(file, mode, 512)
    {
    }
    ...
    ///     If an I/O error ocurred.
    /// </exception>
    protected internal BufferedRandomAccessFile(string name, string mode, int bufferSize) : this(new FileInfo(name),
        mode, bufferSize)
    {
    }
    ...
    /// </summary>
    /// <returns>
    ///     Either <tt>EndianType.BIG_ENDIAN</tt> or
    ...
    public void write(byte b)
    {
        ...
        if (position < byteBuffer.Length)
        {
            ...
            if (position >= maxByte) maxByte = position + 1;
            position++;
    ...
    */
    ...
    protected internal void readNewBuffer(int off)
    {
        ...
        */
        if (byteBufferChanged) flush();
        // Don't allow to seek beyond end of file if reading only
        if (isReadOnly && off >= theFile.Length) throw new EndOfStreamException();
        // Set new offset
        offset = off;
    ...
    /// </summary>
    /// <param name="b">
    ///     The byte to write. The lower 8 bits of <tt>b</tt> are
    ///     written.
    /// </param>
    /// <exception cref="java.io.IOException">
    ///     If an I/O error ocurred.
    ...
    public void write(byte[] b, int offset, int length)
    {
        ...
        for (i = offset; i < stop; i++) write(b[i]);
    }
