...
public class FileFormatWriter
{
    ...
    public virtual int writeFileFormat()
    {
        ...
        {
            throw new ApplicationException("Error while writing JP2 file format(2): " + e.Message + "\n" +
                                           e.StackTrace);
        }
    ...
    public virtual void writeBitsPerComponentBox()
    {
        ...
        // Write bpc fields
        for (var i = 0; i < nc; i++) fi.writeByte(bpc[i] - 1);
    }
    ...
    public virtual void writeColourSpecificationBox()
    {
        ...
        // Write EnumCS field
        if (nc > 1)
            fi.writeInt(FileFormatBoxes.CSB_ENUM_SRGB);
        else
            fi.writeInt(FileFormatBoxes.CSB_ENUM_GREY);
    }
