...
public class FileFormatReader
{
    ...
    public virtual void readFileFormat()
    {
        ...
        try
        {
            ...
            if (in_Renamed.readInt() != 0x0000000c || in_Renamed.readInt() != FileFormatBoxes.JP2_SIGNATURE_BOX ||
                in_Renamed.readInt() != 0x0d0a870a)
            {
                // Not a JP2 file
                in_Renamed.seek(0);
                ...
                if (marker != Markers.SOC)
                    ...
                    throw new ApplicationException("File is neither valid JP2 file nor " +
            ...
            if (!readFileTypeBox())
                ...
                throw new ApplicationException("Invalid JP2 file: File Type box missing");
                ...
                switch (box)
                {
                    case FileFormatBoxes.CONTIGUOUS_CODESTREAM_BOX:
                        if (!jp2HeaderBoxFound)
                            throw new ApplicationException("Invalid JP2 file: JP2Header box not " +
                    ...
                    case FileFormatBoxes.JP2_HEADER_BOX:
                        if (jp2HeaderBoxFound)
                            throw new ApplicationException("Invalid JP2 file: Multiple " + "JP2Header boxes found");
        ...
        {
            throw new ApplicationException("EOF reached before finding Contiguous " + "Codestream Box");
        }

        if (codeStreamPos.Count == 0)
            // Not a valid JP2 file or codestream
            throw new ApplicationException("Invalid JP2 file: Contiguous codestream box " + "missing");
    }
    ...
    public virtual bool readFileTypeBox()
    {
        ...
        if (length == 0)
            ...
            throw new ApplicationException("Zero-length of Profile Box");

        // Check that this is a File Type box (TBox)
        if (in_Renamed.readInt() != FileFormatBoxes.FILE_TYPE_BOX) return false;
    ...
    public virtual bool readJP2HeaderBox(long pos, int length, long longLength)
    {
        if (length == 0)
            // This can not be last box
            throw new ApplicationException("Zero-length of JP2Header Box");
    ...
    public virtual bool readContiguousCodeStreamBox(long pos, int length, long longLength)
    {
        ...
        if (codeStreamPos == null)
            codeStreamPos = ArrayList.Synchronized(new ArrayList(10));
        codeStreamPos.Add(ccpos);

        // Add new codestream length to length vector
        if (codeStreamLength == null)
            codeStreamLength = ArrayList.Synchronized(new ArrayList(10));
